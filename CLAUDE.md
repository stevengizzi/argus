...
| DEF-157 | `evaluation.db` exceeds 4 GB despite DEC-345 7-day retention — missing VACUUM | Unscheduled | Investigation (Apr 20): 937K rows from today, 3.7 GB file, but 94.3% of pages are freelist (923,947/979,337). Retention DELETE ran correctly (cleared 17 days of old data on boot). Root cause: no VACUUM after bulk DELETE — SQLite never reclaims free pages without explicit VACUUM. Fix: add `VACUUM` after `cleanup_old_events()` or enable `PRAGMA auto_vacuum = INCREMENTAL` at table creation. Secondary concern: 937K events in 26 min = ~36K events/min write volume. Priority: MEDIUM. |
