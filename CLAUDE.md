...
> Last updated: April 20, 2026 (Sprint 31.85 doc sync — Parquet cache consolidation, DEF-161 resolved)

## Active Sprint

**Active sprint: Between sprints. 22 shadow variants collecting CounterfactualTracker data. Consolidation script ready; operator repoint of `config/historical_query.yaml` pending. Next: Sprint 31B (Research Console / Variant Factory).**
...
- **Tests:** 4,934 pytest + 846 Vitest (3 pre-existing failures — see Known Issues)
...
| DEF-150 | Flaky pre-existing test: `tests/sprint_runner/test_notifications.py::TestReminderEscalation::test_check_reminder_sends_after_interval` — race condition under `-n auto`, passes in isolation | Unscheduled | Discovered during Sprint 31A.75 S1. Reconfirmed pre-existing during Sprint 31.85 (Tier 2 review). Priority: LOW. |
...
| DEF-162 | Monthly re-consolidation cron scheduling for `scripts/consolidate_parquet_cache.py` — chains with the existing `scripts/populate_historical_cache.py --update` cron (DEF-097). Both should be scheduled as a pair: `--update` pulls new monthly Parquet files, then `consolidate_parquet_cache.py` (default `--resume`) merges only the symbols whose source row count grew. | Opportunistic | Opened Sprint 31.85 doc-sync. Companion to DEF-097. Priority: LOW. |
| DEF-163 | Date-decay test hygiene batch — replace hardcoded dates and `datetime.now()`-seeded fixtures in pre-existing tests that fail as real time advances: `tests/analytics/test_def159_entry_price_known.py::test_get_todays_pnl_excludes_unrecoverable` (seeds bogus trade, expects `pnl == 100.0`, observed `0.0`) and `tests/core/test_regime_vector_expansion.py::TestHistoryStoreMigration::test_history_store_migration` (reconfirmed decay in Sprint 31.85 despite Sprint 32.8 partial fix — likely a second hardcoded constant). Same root-cause class as DEF-137. | Opportunistic | Opened Sprint 31.85 doc-sync. Both confirmed unrelated to Sprint 31.85 changes. Priority: LOW. |
