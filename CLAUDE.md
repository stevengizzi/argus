...
> Last updated: April 23, 2026 (IMPROMPTU-05 — DEF-179 + DEF-180 + DEF-181 closed; `python-jose` → `PyJWT` migrated at 5 production + 3 test sites; `uv`-generated `requirements.lock` + `requirements-dev.lock` landed with CI installing from the dev lockfile; GitHub Actions `checkout`/`setup-python`/`setup-node` all bumped to `@v6` for Node 24 compatibility ahead of the 2026-06-02 deadline; `populate_historical_cache.py` LaCie legacy paths removed; test delta 0 — 5054 → 5054)
