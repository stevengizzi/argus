...
| DEF-190 | pyarrow/xdist concurrent `register_extension_type` race. On first-run of full pytest suite with `-n auto`, two worker processes can race to register `pandas.period` extension type during pyarrow/pandas import, producing `pyarrow.lib.ArrowKeyError: A type extension with name pandas.period already defined` and failing tests in `tests/test_integration_sprint3.py` (and potentially others). Second-run is clean because registration has completed. FIX-09 Tier 2 review observed this; re-run green. Distinct from DEF-171 (ULID module state) — shares "xdist worker isolation" family but different root cause. | FIX-13 | LOW priority. Fix options: (a) conftest.py-level eager pyarrow import before xdist workers fork; (b) session-scoped fixture that serializes first-import across workers; (c) `pytest-xdist --dist loadfile` mode to group tests by file. Prefer (a) — simplest. Add a regression test that imports pyarrow in a specific order. |
...
| DEF-192 | Consolidated test runtime warning cleanup debt. Post-FIX-09 CI produced 26–27 warnings across ~6 categories: (1) aiosqlite "Event loop is closed" on connection-worker-thread shutdown (3 test sites — `test_trade_logger`, `test_vix_pipeline`, `test_runtime_wiring`); (2) coroutine-never-awaited in AsyncMock usage (8 warning sites across `test_engine`, `test_health`, `test_ibkr_broker`, `test_auto_trigger`, `test_executor`); (3) pytest collection warning on `scripts/sprint_runner/state.py:158 class TestBaseline` (pytest tries to collect, `__init__` makes it uncollectable); (4) `websockets.legacy` deprecation; (5) `OrderManager(auto_cleanup_orphans=...)` deprecation in `test_order_manager_reconciliation`; (6) numpy invalid-cast in `vectorbt_afternoon_momentum.py:1103`. All pre-existing, none regressions. | FIX-13 | LOW priority. Fix patterns: aiosqlite → explicit `await db.close()` in fixtures; AsyncMock coroutines → use `AsyncMock` properly or `await` and discard; `TestBaseline` → rename to `Baseline` or add `__test__ = False`; deprecated imports → swap to non-legacy APIs; `OrderManager` kwarg → migrate tests to `reconciliation_config=`; numpy cast → guard with `np.nan_to_num` before `.astype(int)`. Acceptance: full pytest warning count ≤ 5. |
