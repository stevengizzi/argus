...
- **Active sprint:** Between sprints. 22 shadow variants deployed via experiments.yaml. Next: Sprint 31B (Research Console / Variant Factory).
...
| DEF-161 | DuckDB Parquet consolidation needed — 983K individual Parquet files make DuckDB queries unusable (VIEW scans take hours, TABLE materialization takes 16+ hours). Need per-symbol file consolidation (24K files instead of 983K) or summary-table approach. Blocks Research Console SQL queries. `resolve_symbols_fast.py` (pyarrow sampling) is the interim workaround for symbol resolution. | Sprint 31B | Discovered during Sprint 31.75 S4. Priority: MEDIUM. Blocks 31B Research Console DuckDB features. |
