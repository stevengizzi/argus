...
| DEF-163 | Timezone-boundary bug in `test_get_todays_pnl_excludes_unrecoverable` (FIX-05 fix incomplete). FIX-05 ET-aligned the Python-side `now` but `TradeLogger.get_todays_pnl()` still uses SQLite's `date()` function against serialized ISO timestamps. When ET and UTC dates differ (≈ 20:00–24:00 ET daily), `date(exit_time)` extracts the UTC date from the stored ISO string while `today_et` is ET-based, causing ~4-hour daily windows where the test fails. Observed pre-existing failure during FIX-08 close-out + review runs. Other FIX-05-era fixes (`test_history_store_migration`, `_make_vector()` default `computed_at`) remain correctly resolved and are out of scope for the re-open. | FIX-13 | Two options: (a) align `get_todays_pnl` to use UTC `today` to match the stored serialization, updating the test accordingly; (b) serialize `exit_time` with its naive ET string rather than UTC, making `date()` extract the ET date. Option (a) is the minimum-change fix; option (b) matches the ET-everywhere convention used elsewhere. Priority: LOW (pre-existing, ~4h-per-day intermittent). |
