...
> Last updated: April 23, 2026 (FIX-13a audit 2026-04-21 — test-hygiene tactical batch + DEF-150/167/171/190 closures)
...
| DEF-199 | `_flatten_unknown_position()` systematically doubles short positions at EOD. On April 22, 50 of 51 "untracked broker positions" ended the session exactly 2× short (e.g., ARGUS SELL 223 AAL → broker reported −446; ARGUS SELL 2,417 ASPI → broker reported −4,834; 50 total symbols at exactly 2.00× ratio), totaling 34,239 shares requiring manual intervention via `scripts/ibkr_close_all_positions.py`. Root cause traced across three files: (1) `argus/execution/ibkr_broker.py:935` returns position quantity as `shares = abs(int(ib_pos.position))` — long/short distinction preserved on `pos.side`, not on the shares value; (2) `argus/models/trading.py:164` constrains `Position.shares: int = Field(ge=1)` — always ≥1, so `qty > 0` is trivially True; (3) `argus/execution/order_manager.py:1707` filters EOD Pass 2 candidates only on `qty > 0` without checking `pos.side`, so any position (long OR short) passes the filter; (4) `argus/execution/order_manager.py:1936-1947` then unconditionally places a MARKET SELL for `abs(qty)`. Net effect: any short position in `get_positions()` gets SELL-flattened, doubling the short. Same bug also exists at `order_manager.py:1684` (EOD Pass 1 retry block, Sprint 32.9). 50 of 51 positions were likely already short before EOD due to DEF-196 cascade; the PURR 1.00× outlier is likely a DEF-194 stale-cache artifact. A 2.00× ratio across 50 positions is mathematical proof the mechanism is deterministic, not coincidental. On a live account this would be catastrophic. Surfaced in April 22 market session debrief §A1. | **IMPROMPTU-01 (critical path, blocks next paper session)** | CRITICAL / SAFETY. Proposed fix: add `pos.side` check in filter at `order_manager.py:1707` (and `:1684`) — if `pos.side == OrderSide.BUY`, proceed with SELL flatten as-is; if `pos.side == OrderSide.SELL`, emit ERROR log and skip (ARGUS is currently long-only; an unexpected short should halt auto-cleanup pending operator decision, not auto-cover via BUY which has its own risk surface). Regression tests required: (a) mock `IBKRBroker.get_positions()` returning a Position with `side=SELL, shares=100`, assert `_flatten_unknown_position` is NOT called with a SELL for that symbol; (b) end-to-end test with mixed long/short broker positions, verify only longs get flattened. Grep-verify all `get_positions()` call sites for the same blind spot. Startup invariant recommended: after broker connect, assert all positions have `side == BUY`; if any `side == SELL`, WARN + halt auto-startup-cleanup pending operator ack. Tier 2 adversarial review required (execution-layer safety-critical diff). |
