...
| DEF-082 | Quality engine `catalyst_quality` always 50.0 — root cause is CatalystStorage pointing at `argus.db` (empty) instead of `catalyst.db` (12,114 catalysts) per audit P1-D1 C1 | FIX-01-catalyst-db-quality-pipeline | Promoted to Phase 3 fix group in audit 2026-04-21. Previously mis-attributed to "expected neutral default when no catalysts available" — verified by `sqlite3` query showing `argus.db.catalyst_events` has 0 rows, `catalyst.db.catalyst_events` has 12,114 rows. Priority: CRITICAL. |
...
| DEF-142 | Quality engine grade compression — all signals scoring B. Sprint 32.9 recalibration edited `config/quality_engine.yaml` but `load_config()` reads `system_live.yaml`, which still carries pre-recal values. CONFIRMED unresolved 2026-04-21 via live `quality_history` query: 97% of 7-day signals grade B, zero A-grades, max composite 67.0 vs A- threshold 70. | FIX-01-catalyst-db-quality-pipeline | REOPENED by audit 2026-04-21 P1-H4. Original "resolved" mark in Sprint 32.9 S3 was misapplied — YAML edit never reached runtime. Bundled with FIX-01 per triage recommendation (the two fixes land together). Priority: CRITICAL. |
