...
> Last updated: April 22, 2026 (FIX-14 audit 2026-04-21 — primary context doc compression)
...
- **Date-decay/timezone test failures (DEF-163):** 2 tests embed hardcoded dates / timezone-boundary assumptions. Batched into FIX-13. Priority: LOW.
...
- **Tests:** 4,985 pytest + 859 Vitest (as of FIX-04). Known flakes: DEF-150 (time-of-day arithmetic, first 2 min of every hour) + DEF-163 (timezone-boundary + hardcoded dates) + DEF-171 (ibkr_broker xdist). All batched into FIX-13. Treat as pre-existing.
...
| DEF-014 | SystemAlertEvent for dead data feed | Health Monitor alerting built | `DatabentoDataService._run_with_reconnection()` logs `critical` but emits no event. |
...
| DEF-032 | FMPScannerSource criteria_list filtering | Quality Engine provides scoring criteria | `scan()` accepts `criteria_list` but ignores it. |
...
| DEF-037 | FMP API key redaction in error logs | Next cleanup sprint | FMP URLs with `apikey=XXX` appear in error logs. Priority: MEDIUM. |
...
| DEF-091 | Public accessors on V1 RegimeClassifier + VIXDataService private attrs | Unscheduled | **PARTIALLY RESOLVED (FIX-11, 2026-04-21):** API-side public API in place (`attach_vix_service()` + calculator properties). Remaining: V2 access to V1 `trend_score`/vol thresholds + `VIXDataService._config` read by `RegimeClassifierV2.__init__`. Priority: LOW. |
| DEF-092 | Unused Protocol types in regime.py | Unscheduled | V2 uses concrete types; Protocol defs unused. Priority: LOW. |
...
| DEF-096 | Protocol type for duck-typed candle store reference | Unscheduled | Currently `object` + `hasattr()`. Protocol restores type safety without circular imports. Priority: LOW. |
...
| DEF-104 | Dual ExitReason enums (events.py + trading.py) sync | Unscheduled | Consolidation candidate. Priority: MEDIUM. |
...
| DEF-123 | `build_parameter_grid()` float accumulation | Unscheduled | Mitigated by round + dedup; should use integer-stepping or numpy.arange. Cosmetic. Priority: LOW. |
...
| DEF-150 | Time-of-day arithmetic bug in `test_check_reminder_sends_after_interval` | FIX-13 | `(minute - 2) % 60` breaks when minute ∈ {0,1}. One-line fix: `datetime.now(UTC) - timedelta(minutes=2)`. Priority: LOW. |
...
| DEF-163 | Timezone-boundary bugs + hardcoded Vitest dates | FIX-13 | 3 root causes: (a) `test_get_todays_pnl_excludes_unrecoverable` UTC vs ET, (b) 2nd hardcoded constant in `test_history_store_migration` L36, (c) 4 Vitest files with absolute dates. Priority: LOW. |
...
| DEF-165 | DuckDB conn close hangs when CREATE VIEW interrupted | Unscheduled | Candidate fix: `conn.interrupt()` before `conn.close()`. Only manifests during DEF-164 scenario. Priority: LOW. |
| DEF-166 | `test_speed_benchmark` flaky under pytest-cov | Unscheduled | Audit P1-H4. Not blocking. Priority: LOW. |
| DEF-167 | Vitest hardcoded-date decay in 4 files | FIX-13 | **PARTIALLY RESOLVED** (FIX-11, 2026-04-21): the 4 flagged files converted to `new Date().toISOString()`. Scan remainder during FIX-13. Priority: LOW. |
...
| DEF-170 | VIX regime calculators stay None in production | FIX-05-core | `main.py:770` constructs `RegimeClassifierV2` without vix_data_service. Fix: either reorder construction, or have `attach_vix_service()` re-instantiate calculators. Priority: MEDIUM — production VIX regime inert. |
| DEF-171 | `test_all_ulids_mapped_bidirectionally` xdist flake | FIX-13 | Observed 3× in Sprint 31.9 reviews. Distinct from DEF-150/163. Likely module-level state not reset between xdist workers. Priority: LOW. |
...
| DEF-177 | `RejectionStage` enum missing `MARGIN_CIRCUIT`. Margin-circuit-breaker rejections currently emit `SignalRejectedEvent(rejection_stage="risk_manager")`, conflating them with ordinary risk-manager rejections in the `FilterAccuracy.by_stage` cut (the principal cut used by Experiments and Learning surfaces). FIX-04 (audit 2026-04-21, P1-D1-M03) identified the fix but deferred it: adding `MARGIN_CIRCUIT = "margin_circuit"` requires editing `argus/intelligence/counterfactual.py` (the enum), updating `counterfactual_positions.rejection_stage` schema/values, and bumping the emitted stage at `argus/execution/order_manager.py:485`. Scope spans execution + intelligence domains and was outside FIX-04's execution-only declared scope (halt-rule-4). | FIX-06 or dedicated cross-domain session | Cross-domain fix: (1) extend `RejectionStage` enum in `argus/intelligence/counterfactual.py` with `MARGIN_CIRCUIT`; (2) confirm `RejectionStage(event.rejection_stage)` call at `argus/main.py:1833` parses "margin_circuit" via StrEnum — additive, no existing consumer breaks; (3) change `order_manager.py:485` `rejection_stage="risk_manager"` → `rejection_stage="margin_circuit"` inside the margin-circuit block; (4) add regression test asserting the emitted stage differs under margin open vs ordinary risk rejection. Priority: MEDIUM — masks operational signal during margin incidents. |
