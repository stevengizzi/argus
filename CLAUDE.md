...
> Last updated: April 23, 2026 (IMPROMPTU-06 — test-debt & warning-cleanup bundle; DEF-048/DEF-049/DEF-166/DEF-176/DEF-185 closed, DEF-192 moved to PARTIAL. `OrderManager.auto_cleanup_orphans=` legacy kwarg + guard + docstring deleted after migrating 3 reconciliation test files to `ReconciliationConfig`; 5 `assert isinstance` sites across `argus/analytics/ensemble_evaluation.py` (×3) + `argus/intelligence/learning/outcome_collector.py` (×2) rewritten as `raise TypeError` guards with 5 revert-proof regression tests; `test_main.py` fixture repaired — 4 root causes identified (missing `api.enabled: false`, missing `exit_management.yaml`, `broker.get_positions` not AsyncMock, `data_dir: "data"` pointing at repo's 13GB data/) collapsed 21 failures to 5 residual xdist flakes which are now skipped with explicit DEF-048 reasons; full test_main.py state now 39 pass / 5 skip / 0 fail under both isolated and `-n auto`; warning count 43 → 26 (-40%). Test delta: 5054 → 5057 (+3) on `--ignore=tests/test_main.py`; test_main.py 23 → 39 pass + 5 skip.)
