...
| ~~DEF-163~~ | ~~Timezone-boundary bugs + hardcoded Vitest dates~~ | — | **RESOLVED** (FIX-05, audit 2026-04-21). Python-side: `test_get_todays_pnl_excludes_unrecoverable` ET-aligns `now`; `test_history_store_migration` captures dates via ET; `_make_vector()` default `computed_at` is now `datetime.now(UTC) - timedelta(hours=1)` rather than hardcoded March 26 2026. Vitest hardcoded-date scan remains under DEF-167 (FIX-13 scope). |
