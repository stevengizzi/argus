...
> Last updated: April 23, 2026 (IMPROMPTU-07 — doc-hygiene + small-ops + UI-cosmetic bundle; DEF-164/DEF-169/DEF-189/DEF-191/DEF-198 closed + Apr 21 debrief F-05/F-06/F-08 retired. Boot-grace shutdown suppression (`OrderManagerConfig.auto_shutdown_boot_grace_minutes=10`) added; `revalidate_strategy.py` config_overrides now translate VectorBT names to Pydantic field names + Pydantic-model-field validation; trade/decision log ULIDs widened 8→12 chars; `/counterfactual/positions` REST serializes R-multiple `mfe_r`/`mae_r` alongside preserved dollar `max_*_excursion` fields; Shadow Trades UI renders from R-multiples; `PRIORITY_BY_WIN_RATE` warning downgraded to DEBUG; 6 sprint-archaeology inline comments removed from `argus/main.py`; `getStrategyDisplay` / Tailwind accessors resolve experiment-variant IDs to their base strategy for badge colors; architecture.md §3.9 + FIX-03 closeout + start() docstring corrected from "17 phases" → "19 phases" (12 primary + 7 sub). Test delta: 5057 → 5073 (+16 pytest — 8 DEF-189, 4 DEF-164, 2 F-05, 2 F-06 API); 859 → 866 Vitest (+7 — 6 strategy-variant resolution, 1 F-06 UI).)
...
- **Tests:** 5,057 pytest (--ignore=tests/test_main.py) + 39 pass / 5 skip on tests/test_main.py + 859 Vitest. Known flakes: DEF-150 (time-of-day arithmetic, first 2 min of every hour) + DEF-167 (Vitest hardcoded-date scan) + DEF-171 (ibkr_broker xdist) + DEF-190 (pyarrow/xdist register_extension_type race) + DEF-192 (runtime warning cleanup debt, non-blocking, now at 26 warnings post-IMPROMPTU-06). All batched into FIX-13 or IMPROMPTU-06. Treat as pre-existing.
