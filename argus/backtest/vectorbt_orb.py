...
def _simulate_trades_for_day_slow(
    ...
    """Simulate a single trade for one day (SLOW VERSION - kept for validation).
    ...
    Args:
        ...
        stop_buffer_pct: Stop buffer as percentage (e.g., 0.1 for 0.1%).
        target_r: Profit target as R-multiple.
        max_hold_minutes: Maximum hold time before time stop.

    Returns:
        Dict with trade details (entry_price, exit_price, r_multiple, hold_minutes,
        exit_reason) or None if no trade triggered.
    """
    # Bars after OR completes
    post_or_bars = day_bars[day_bars["bar_number_in_day"] > or_complete_bar].copy()

    if post_or_bars.empty:
        return None

    # Look for breakout: first bar that closes above OR high
    breakout_bars = post_or_bars[post_or_bars["close"] > or_high]
    ...
    entry_bar_idx = entry_bar["bar_number_in_day"]
    entry_minutes = entry_bar["minutes_from_open"]
    ...
    post_entry_bars = day_bars[day_bars["bar_number_in_day"] > entry_bar_idx].copy()

    exit_price = None
    exit_reason = None
    exit_minutes = None

    for _, bar in post_entry_bars.iterrows():
        bar_minutes = bar["minutes_from_open"]
        hold_minutes = bar_minutes - entry_minutes

        # Check EOD flatten (3:45 PM ET = 375 minutes from open)
        if bar_minutes >= 375:
            exit_price = bar["close"]
            exit_reason = "eod"
            exit_minutes = hold_minutes
            break
    ...
    # If no exit found (shouldn't happen with EOD check, but safety)
    if exit_price is None:
        # Use last bar of day
        last_bar = day_bars.iloc[-1]
        exit_price = last_bar["close"]
        exit_reason = "eod"
        exit_minutes = last_bar["minutes_from_open"] - entry_minutes
...
_simulate_trades_for_day = _simulate_trades_for_day_slow
