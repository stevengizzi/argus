...
class WalkForwardConfig:
    ...
    # Red-to-Green VectorBT IS path was retired by FIX-09 P1-E2-M03 —
    # the branch was unreachable via revalidate_strategy.py's
    # ``_WALK_FORWARD_SUPPORTED`` set. R2G backtesting remains available
    # via BacktestEngine (scripts/revalidate_strategy.py --strategy red_to_green
    # runs the engine-only path).
...
async def optimize_in_sample(
    ...
    if config.strategy == "orb_scalp":
        ...
        # R2G walk-forward retired by FIX-09 P1-E2-M03; use BacktestEngine
        # directly via scripts/revalidate_strategy.py for R2G validation.
...
async def validate_out_of_sample(
    ...
    if config.strategy == "orb_scalp":
        ...
        # R2G OOS walk-forward retired by FIX-09 P1-E2-M03; use
        # BacktestEngine directly via scripts/revalidate_strategy.py.
...
# R2G removed (FIX-09 P1-E2-M03): walk_forward.py no longer drives R2G
# IS optimization, so the OOS BacktestEngine path is also unreachable
# via this module. R2G backtesting is still exposed through the engine-only
# path in ``scripts/revalidate_strategy.py``.
...
def _build_config_overrides(
    ...
    # R2G branch retired by FIX-09 P1-E2-M03.
