 ...
 * Containers and labels are stable; only data values transition during period changes.
 */
...
export function MetricsGrid({ metrics, isTransitioning = false, className = '' }: MetricsGridProps) {
  ...
  return (
    <Card className={className}>
      <div
        className={`grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 transition-opacity duration-200 ${
      ...
      <div
        className={`hidden md:grid grid-cols-4 lg:grid-cols-6 gap-4 mt-4 pt-4 border-t border-argus-border transition-opacity duration-200 ${
          isTransitioning ? 'opacity-40' : 'opacity-100'
        }`}
      >
