...
| **Status** | Active |
| **Amendment 1 (FIX-07, audit 2026-04-21, P1-D1-M12)** | Dedup anchor semantics pinned: each new catalyst is compared to the **last kept** item in the group (`kept[-1]`), NOT to a sliding cluster midpoint or the first-seen anchor. Concrete walk example — A(t=0, score=70) → B(t=20, score=50) → C(t=40, score=60) with window=30: A kept, B dropped (A is within 30 and scores higher), C compared to A (diff=40 > 30) → C also kept. Both A and C survive even though C is within 20 minutes of B's original timestamp. Trade-off: when scores *decrease* within a window, dedup is slightly under-counted in the upper-scoring direction; when scores *increase*, the kept item moves forward and a slightly later arrival may land outside the new window even though it was in-window against the first arrival. Real-world catalysts within a 30-min bucket have similar scores, so the effect is marginal. Option (c) "explicitly document current behavior" was chosen over (a) cluster-midpoint or (b) first-seen-anchor because changing the semantic would alter dedup counts across ongoing paper-trading data collection. Pinned by regression test `tests/intelligence/test_fix07_audit.py::TestSemanticDedupAnchor`. |
...
DEC range allocation: DEC-385–DEC-388 reserved during Sprint 31.91 planning. Final allocation:

- **DEC-385** — Side-Aware Reconciliation Contract (Sessions 2a / 2b.1 / 2b.2 / 2c.1 / 2c.2 / 2d). MATERIALIZED at S2d 2026-04-02; written below at sprint-close (D14, 2026-04-28) per Pattern B per `protocols/mid-sprint-doc-sync.md` v1.0.0.
- **DEC-386** — OCA-Group Threading + Broker-Only Safety (Sessions 0 / 1a / 1b / 1c). Written below following Tier 3 architectural review #1 (PROCEED, 2026-04-27). Verdict artifact at `docs/sprints/sprint-31.91-reconciliation-drift/tier-3-review-1-verdict.md`.
- **DEC-387** — FREED at Tier 3 #1 close (reserved during planning but not consumed).
- **DEC-388** — Alert Observability Architecture (Sessions 5a.1 / 5a.2 / 5b + Impromptus A+B+C + Sessions 5c / 5d / 5e). Resolves DEF-014 (PRIMARY DEFECT, October 2025). MATERIALIZED at S5e 2026-04-28; written below at sprint-close (D14, 2026-04-28) per Pattern B per Tier 3 #2 amended verdict 2026-04-28.
...
| Field | Value |
|-------|-------|
| **Date** | 2026-04-28 (sprint-close materialization; behavior LIVE since S2d 2026-04-02) |
| **Sessions** | Sprint 31.91 — Sessions 2a, 2b.1, 2b.2, 2c.1, 2c.2, 2d |
| **Final commit (sprint-close)** | Sprint 31.91 D14 doc-sync (current commit). Behavior live on `main` since S2d landed (`9b7246c`/`b25b419`/`6009397`/`49beae2` for Track A; subsequent S2*-track commits for Track B). |
| **Tier 3 verdict** | Sprint-close materialization per Pattern B (`protocols/mid-sprint-doc-sync.md` v1.0.0). Architectural soundness verified across in-flight Tier 2 reviews of S2a–S2d (all CLEAR). |
| **Context** | Pre-Sprint-31.91, ARGUS reconciliation logic in OrderManager (Pass 1 startup, Pass 2 EOD) treated all unconfirmed positions identically regardless of broker-reported side. This was correct for ARGUS's long-only design assumption (DEC-166) — but it meant phantom-short cascades (DEF-204, mechanism diagnostic at `docs/sprints/sprint-31.9/IMPROMPTU-11-mechanism-diagnostic.md`) were "reconciled" by being treated as legitimate positions to track, perpetuating the cascade across daily reconciliation cycles. The mechanism diagnostic identified 3 detection-blindness sites: (1) `reconcile_positions()` orphan loop only acted on ARGUS-orphan direction, silently ignoring broker-orphan / phantom-short direction; (2) `main.py` call site stripped side info before reconciliation runs; (3) `_check_flatten_pending_timeouts` (the DEF-158 retry path) read `broker_qty = abs(int(...))` and issued SELL even when broker was already short — actively doubling the short. |
| **Decision** | Adopt a 6-layer side-aware reconciliation contract: **(L1, S2a)** `argus/main.py` call-site builds typed dict from `broker.get_positions()` including side; reconciliation receives side as first-class field. Replaces `qty = float(getattr(pos, "shares", 0))` (DEF-139 silent-default anti-pattern) with explicit side-typed payload. **(L2, S2b.1)** `SystemAlertEvent.metadata: dict[str, Any] \| None` schema field added — provides forensic surface for side, shares, stage, and other side-aware classification metadata. Cross-references DEF-213 (closed by this layer + S5a.1's atomic emitter migration). **(L3, S2b.2)** `OrderManager` Pass 1 retry path threads side through; SELL detection branching distinguishes broker LONG (legitimate ARGUS-tracked) vs broker SHORT (phantom-short candidate); 3-branch gate replaces unconditional SELL retry. **(L4, S2c.1)** Phantom-short gate emits side-aware `phantom_short_gated_symbol` classifications and persists to `phantom_short_gated_symbols` SQLite table (separate-DB pattern per DEC-345); operator can audit which symbols were gated and why. New `phantom_short_retry_blocked` CRITICAL alert taxonomy when retry is blocked by a phantom-short broker state. **(L5, S2c.2)** EOD verify polling (Pass 2) treats SHORT-side positions as phantom-short candidates rather than long-position reconciliation targets; `_phantom_short_clear_cycles` auto-clear threshold (default 5 cycles) is the conservative time-decay window before allowing re-engagement. **(L6, S2d)** Audit-log enrichment via `phantom_short_override_audit` SQLite table records every operator override for forensic-grade replay; combined with `phantom_short_gated_symbols` and `alert_acknowledgment_audit` (S5a.1), the operator has a complete audit trail of side-aware classification decisions. |
| **Alternatives** | (A) Single-layer fix at the `_check_flatten_pending_timeouts` site only — rejected: closes one of three blindness sites; the broker-orphan direction in `reconcile_positions()` would still silently track phantom shorts. (B) Make `reconcile_positions()` issue COVER orders for broker SHORTs — rejected: ARGUS is long-only by design (DEC-166); short-cover infrastructure deferred to Sprint 30. The correct posture pre-Sprint-30 is alert-and-skip on shorts, not auto-cover. (C) Strip side-tracking entirely and rely on broker reconciliation — rejected: defeats the entire purpose of `_managed_positions` and breaks ARGUS's exit accounting (which assumes long-only). (D) Implement side-awareness in a single mega-session rather than 6 layers — rejected: each layer lands a self-contained safety property and a discrete regression test; the Tier 2 review surface for a mega-session would be unmanageable, and partial rollback (if any layer needed amendment) would force re-touching unrelated layers. |
| **Rationale** | DEC-385 is the structural-detection counterpart to DEC-386's structural-prevention (OCA-group threading at the broker layer). DEC-386 closes ~98% of the cascade mechanism by preventing phantom shorts from being created via OCA-cancellation races. DEC-385 closes the residual ~2% (positions that escape OCA via reconnect-recovery, broker-only events, or operator manual orders) by making them visible and actionable rather than silently absorbed. The two decisions together compose into a single architectural closure of DEF-204; either alone would leave a meaningful safety gap. The 6-layer layering across S2a–S2d makes each step self-contained and individually testable; the regression-checklist invariant 17 monotonic-safety matrix verifies row-by-row that each layer strictly adds capability without removing any. |
| **Impact** | Reconciliation now correctly distinguishes ARGUS-managed long positions from broker-side anomalies. Combined with DEC-386 (OCA-Group Threading + Broker-Only Safety) which closes the upstream OCA mechanism, ~98% of DEF-204's cascade mechanism is now structurally closed and the remaining ~2% (broker-only events) is now visible and audited. Operator's daily-flatten mitigation (`scripts/ibkr_close_all_positions.py`) cessation criteria #1 + #2 + #3 SATISFIED post-S4; #4 (sprint sealed) MET at D14; #5 (5 paper sessions clean post-seal) pending. New SQLite audit tables (`phantom_short_gated_symbols`, `phantom_short_override_audit`) provide forensic replay capability for any future phantom-short event. New `phantom_short_retry_blocked` CRITICAL alert type integrated with DEC-388 alert observability pipeline (HealthMonitor consumer + `POLICY_TABLE` entry + frontend banner). Sprint 31.93 (DEF-211 D1+D2+D3) inherits the mid-session-reconnect counterpart of L1's `reconstruct_from_broker` startup-only contract via `ReconstructContext` parameter and IMPROMPTU-04 startup invariant gate refactor. |
| **Cross-References** | DEF-204 (closed by mechanism alongside DEC-386); DEF-158 (extended scope — flatten retry side-blindness — RESOLVED via Sprint 31.91 S3 anchor `a11c001`); DEF-213 (closed by L2 schema + S5a.1 atomic emitter migration); DEC-117 (atomic bracket orders — preserved byte-for-byte); DEC-345 (separate-DB pattern for `phantom_short_gated_symbols` and `phantom_short_override_audit`); DEC-369 (broker-confirmed reconciliation — preserved); DEC-372 (stop-retry caps — preserved); DEC-386 (OCA architecture — complementary structural closure); DEC-388 (alert observability — `phantom_short_retry_blocked` consumer + audit trail surfaces); DEF-211 D1+D2+D3 (Sprint 31.93 reconnect-recovery counterpart); DEF-194/195/196 (Sprint 31.93 reconnect-recovery cluster); RSK-DEC-386-DOCSTRING (docstring contract on `reconstruct_from_broker` — paired with L1's call-site refactor). Session closeouts: `docs/sprints/sprint-31.91-reconciliation-drift/session-2{a,b.1,b.2,c.1,c.2,d}-closeout.md`. |
| **Status** | Active. |
...
| Field | Value |
|-------|-------|
| **Date** | 2026-04-28 (sprint-close materialization; backend behavior LIVE since S5b 2026-04-28; frontend complete at S5e 2026-04-28) |
| **Sessions** | Sprint 31.91 — Sessions 5a.1, 5a.2, 5b + Impromptus A, B, C + Sessions 5c, 5d, 5e |
| **Final commit (sprint-close)** | Sprint 31.91 D14 doc-sync (current commit). Backend hardening anchor `e78a994` (Impromptu A); producer wiring anchor `8efa72e` (Impromptu B); migration framework universal adoption anchor `3fefda8` (Impromptu C); frontend Layer 1 anchor `3197472` (S5c); frontend Layer 2 anchor `66d0b04` (S5d); frontend Layer 3 anchor `7efd0a0` (S5e); post-S5e catalog freshness hotfix anchor `4c737d5`. |
| **Tier 3 verdict** | PROCEED-with-conditions, AMENDED 2026-04-28 (Tier 3 #2). Sprint-close materialization per Pattern B per amended verdict — DEC-388 deferred because cross-references DEFs being resolved by Impromptus A+B+C and S5c-S5e. Verdict artifact at `docs/sprints/sprint-31.91-reconciliation-drift/tier-3-review-2-verdict.md`. |
| **Context** | Pre-Sprint-31.91, ARGUS had no consumer-side architecture for the ~11 alert types emitted across HealthMonitor, OrderManager, IBKRBroker, and Databento data services. DEF-014 surfaced this in October 2025 as a structural gap — `SystemAlertEvent` was emitted but had no central consumer, no persistence, no UI surface. The system was operationally blind to its own alarms. FIX-06 (audit 2026-04-21) had wired the Databento dead-feed emitter side but explicitly deferred consumer + Command Center surface because no `HealthMonitor` consumer existed. Apr 22-24 paper-session debriefs (DEF-204 cascade discovery) reinforced the urgency: when phantom-short alerts started firing, there was no operator-visible surface to acknowledge them. |
| **Decision** | Adopt a multi-emitter consumer pattern with HealthMonitor as central consumer, spanning 5 architectural layers: **(L1 Producer, S5b + Impromptu B)** 15 emitter sites total across HealthMonitor (eod_residual_shorts + eod_flatten_failed at S5a.1; phantom_short + phantom_short_retry_blocked at S2c.1/S3), OrderManager (cancel_propagation_timeout at S1c), IBKRBroker (`_reconnect()` end-of-retries → `ibkr_disconnect`; `_on_error()` CRITICAL non-connection else-branch → `ibkr_auth_failure`), and DatabentoDataService (Databento dead-feed alert + `DatabentoHeartbeatEvent` with stale-suppression contract via existing `_stale_published`). All sites populate `SystemAlertEvent.metadata: dict[str, Any] \| None` (DEC-385 L2 schema + DEF-213 closure). **(L2 Consumer, S5a.1 + S5a.2)** HealthMonitor subscribes to `SystemAlertEvent` plus 4 predicate handlers (`OrderFilledEvent`, `IBKRReconnectedEvent`, `DatabentoHeartbeatEvent`, `ReconciliationCompletedEvent`). Per-alert-type auto-resolution policy table (`POLICY_TABLE` in `argus/core/alert_auto_resolution.py`, 13 entries post-Impromptu A) maps alert types to predicate functions that determine when an alert can be auto-resolved. AST regression guard (`tests/api/test_policy_table_exhaustiveness.py`) enforces table exhaustiveness against producer-side drift. **(L3 Storage, S5a.1 + S5a.2 + Impromptu C)** SQLite `data/operations.db` with 5 tables: `phantom_short_gated_symbols` (S2c.1), `phantom_short_override_audit` (S2d), `alert_acknowledgment_audit` (S5a.1; audit_kinds: ack/duplicate_ack/late_ack/auto_resolution), `alert_state` (S5a.2 + 3 indexes), `schema_version` (S5a.2; migration framework version tracking). Restart recovery via `rehydrate_alerts_from_db()` reconstructs `_alerts_active` map from `alert_state` rows on startup. The migration framework introduced at S5a.2 was extended at Impromptu C to all 8 ARGUS SQLite DBs (sprint-spec D16 fulfilled). **(L4 REST + WebSocket, S5a.1 + S5a.2 + S5e RULE-007 scope expansion)** REST: `GET /api/v1/alerts/active`, `GET /api/v1/alerts/history`, `POST /api/v1/alerts/{alert_id}/acknowledge` (atomic + idempotent: 200/404/200-late-ack); plus S5e-added `GET /api/v1/alerts/{alert_id}/audit` (audit trail per alert for Observatory detail view). WebSocket: `/ws/v1/alerts` JWT-authenticated; emits `auth_success` → `snapshot` → 4 lifecycle deltas (`alert_active`, `alert_acknowledged`, `alert_auto_resolved`, `alert_archived` defensive forward-compat though no producer exists today). **(L5 Frontend, S5c + S5d + S5e)** Layer 1 (S5c): `useAlerts` TanStack-Query-plus-WebSocket-hybrid hook with reconnect-gated refetch via `wasDisconnectedRef`; `AlertBanner` renders `severity === 'critical' && state === 'active'` only. Layer 2 (S5d): `AlertToast` + `AlertToastStack` (queue cap at 5, oldest-dropped); `AlertAcknowledgmentModal` (reason ≥10 chars validated, role=dialog accessibility, duplicate-ack via `acknowledged_by` mismatch detection — backend returns 200 with original acknowledger preserved on idempotent path, no 409). Layer 3 (S5e): `AlertsPanel` Observatory browsing surface (active + history tables, filters by severity/source/symbol, sortable, date-range picker default last-7-days UTC, `AlertDetailView` modal with metadata + audit trail). `AlertBanner` + `AlertToastStack` mounted at `AppShell.tsx` layout level for cross-page persistence (regression invariant 17 structurally pinned by `AppShell.alerts.test.tsx:163`). 5 in-Dashboard banner mounts + 5 in-Dashboard toast mounts removed at S5e. Severity policy: banner displays `critical` only; `warning`/`info` are toast-only. Per-alert-type `PolicyEntry.operator_ack_required` is canonical home for severity-based gating (replaced removed `AlertsConfig.acknowledgment_required_severities` field per DEF-220 Option A REMOVAL). |
| **Alternatives** | (A) Push-only WebSocket with no REST — rejected: REST `/active` + `/history` are needed for cold-page-load and date-range browsing; without them, every Observatory navigation would require a full WebSocket re-snapshot. (B) Per-alert-type `Event` subclasses instead of `SystemAlertEvent.metadata` typed dict — rejected: would explode the event class hierarchy and force consumer code to switch on type rather than dispatch via the `POLICY_TABLE`. The typed dict approach scales linearly with alert types; subclass approach scales quadratically with consumers. (C) In-memory state without restart recovery — rejected: a restart during an active alert window would silently drop the operator-visible state, and the operator would have no way to retrieve the alert that fired before the restart. (D) Wire `AlertsConfig.acknowledgment_required_severities` field as the canonical ack-gating control — rejected per DEF-220 Option A REMOVAL: `PolicyEntry.operator_ack_required` already provides per-alert-type granularity; the global severity-list field was redundant and confusing. (E) Auto-resolution via global timeout — rejected: each alert type has different lifecycle semantics (`ibkr_disconnect` clears on `IBKRReconnectedEvent`, `databento_dead_feed` clears on `DatabentoHeartbeatEvent`, `eod_residual_shorts` requires operator acknowledgment with no auto-clear). The per-alert-type predicate table is the only correct shape. |
| **Rationale** | The 5-layer architecture mirrors the data-flow direction (producer → consumer → storage → API → UI) and lets each layer be tested in isolation while still composing into an end-to-end pipeline. The HealthMonitor consumer is the "central nervous system" pattern (consistent with how HealthMonitor already aggregates `SystemHealth` snapshots across data + execution + intelligence layers). The `POLICY_TABLE` is the architectural decoupling point that lets producers and consumers evolve independently without recompiling each other; the AST regression guard at `tests/api/test_policy_table_exhaustiveness.py` enforces the contract at test time so producer-side drift cannot silently accumulate. The migration framework adoption at S5a.2 (foundational for `operations.db`) was intentionally scoped to one DB so the pattern could be validated; Impromptu C then applied it to all 8 DBs once the pattern was proven. The `AppShell.tsx` cross-page mount (S5e) is the architectural correction to S5c+S5d's per-page mount pattern — critical alerts cannot be missed regardless of which Command Center page is active. |
| **Impact** | DEF-014 FULLY RESOLVED. ARGUS now has end-to-end alert observability from emit through operator acknowledgment with audit trail. The Observatory's `/observatory/alerts` toggle overlay provides historical browsing; the cross-page banner ensures critical alerts cannot be missed regardless of which Command Center page is active. Tier-3-#2-spawned defects: 8 of 9 RESOLVED-IN-SPRINT (DEF-217/218/219/220/221/223/224/225); 1 deferred (DEF-222 — subscribe-before-rehydrate audit, bounded by first producer-wiring sprint introducing producer for `ReconciliationCompletedEvent` / `IBKRReconnectedEvent` / `DatabentoHeartbeatEvent`). 5 new opportunistic DEFs filed with clear triggers: DEF-226 (focus-trap), DEF-227 (auth context wiring, BLOCKED), DEF-228 (`/history` `until` query parameter), DEF-229 (Observatory pagination virtualization), DEF-230 (audit-loading state). Migration framework now spans all 8 ARGUS SQLite DBs (Impromptu C, sprint-spec D16 fulfilled) — sets foundation for any future schema evolution. Catalog hotfix `4c737d5` post-S5e surfaced sprint-wide observation about DEC-328 full-suite-at-Tier-1 process discipline (folded into `process-evolution.md` F.1). |
| **Cross-References** | DEF-014 (closed); DEF-217/218/219/220/221/223/224/225 (Tier 3 #2 spawned, all closed); DEF-222 (deferred to first producer-wiring sprint); DEF-226/227/228/229/230 (opportunistic LOW deferrals); DEC-385 (parallel architectural closure for reconciliation drift; DEC-385 L2 metadata schema is DEC-388's structural foundation); DEC-386 (parallel architectural closure pattern for DEF-204); DEC-345 (separate-DB pattern for `operations.db`); DEC-328 (test discipline; lesson learned at post-S5e catalog hotfix — surfaced as F.1 in `process-evolution.md`). Session closeouts: `docs/sprints/sprint-31.91-reconciliation-drift/session-{5a.1,5a.2,5b,5c,5d,5e}-closeout.md` + `impromptu-{a,b,c}-closeout.md`. Architecture reference: `docs/architecture.md` §14 Alert Observability Subsystem. |
| **Status** | Active. |
...
DEC range allocation: DEC-389 written below.

### DEC-389 | Config-Driven `evaluation.db` Retention Policy + Observability Subfields

| Field | Value |
|-------|-------|
| **Date** | 2026-04-28 (Sprint 31.915 Session 1) |
| **Sessions** | Sprint 31.915 — Session 1 (single-session impromptu, full-protocol) |
| **Decision** | (1) Retention + observability policy for `data/evaluation.db` is supplied via a new `EvaluationStoreConfig` Pydantic model in `argus/core/config.py`, exposed on `SystemConfig.evaluation_store` and loaded from `config/evaluation_store.yaml` via the standalone-overlay registry (`_STANDALONE_SYSTEM_OVERLAYS`, DEC-384 / FIX-02 pattern). Default `retention_days = 2` (was implicit 7 from IMPROMPTU-10's hardcoded class constant) — aligns steady-state ~10 GB with operator's disk environment. (2) `EvaluationEventStore.cleanup_old_events()` emits its deletion-INFO line BEFORE `await self._vacuum()` so a vacuum failure cannot eat the deletion record (Phase A H3 mechanism); zero-deletion path always logs (`retention scanned (cutoff X, 0 rows matched)`); observability instance fields (`_last_retention_run_at_et`, `_last_retention_deleted_count`) update regardless of branch. (3) `EvaluationEventStore._vacuum()` includes a non-bypassable pre-VACUUM disk-headroom check (`shutil.disk_usage(volume).free >= pre_vacuum_headroom_multiplier × current_db_size`, default 2.0×) per RULE-039 — VACUUM aborts loudly when headroom is insufficient rather than silently ENOSPC'ing. (4) `/health` JSON exposes `evaluation_db.{size_mb, last_retention_run_at_et, last_retention_deleted_count, freelist_pct}` via `HealthMonitor.register_evaluation_store()` + `get_evaluation_db_health()` (DEF-233). |
| **Rationale** | Three coupled concerns surfaced in the Apr 28 disk-pressure investigation: (a) IMPROMPTU-10's periodic-task fix had an unknown silent-failure mode (Apr 27→28 freelist transition 0.0% → 0.9% with no `retention deleted N rows` INFO line emitted); (b) `RETENTION_DAYS = 7` was incompatible with operator's 460 GB disk environment under ~5 GB/day evaluation telemetry ingestion, leaving insufficient headroom alongside Parquet caches and 7 sibling SQLite stores; (c) the retention loop had zero `/health`-surfaced visibility. A single hot-patch addressing only (b) would ship the silent-failure mechanism intact. The Phase A diagnostic (`dev-logs/2026-04-28_retention-mechanism-diagnostic.md`) conclusively ruled out the prompt's primary hypothesis (H1, aiosqlite `cursor.rowcount` post-commit) — `rowcount` returns the correct deleted-row count BOTH before and after `commit()` on the in-tree driver — and confirmed H3: production silent-failure mode was that `await self._vacuum()` raised mid-cleanup, propagating up and skipping the success-path INFO line that previously lived AFTER the VACUUM call. The G3 fix (deletion-INFO before VACUUM + zero-deletion logged) closes the observability gap; the G4 fix (pre-VACUUM headroom check) prevents the silent-ENOSPC class structurally; the G5 fix (`/health` subfield) gives operators independent confirmation of retention liveness. |
| **Alternatives Considered** | (i) Hot-patch lower `RETENTION_DAYS = 2` only, leaving silent-failure mode intact — REJECTED, would re-incident at next disk-pressure event. (ii) Capture `cursor.rowcount` BEFORE commit per the prompt's H1 hypothesis — REJECTED, Phase A ruled this out. (iii) Move `logger.info(...)` AFTER `await self._vacuum()` per the prompt's prescribed Phase B fix shape — DEVIATED FROM, because Phase A confirmed H3 (not H1); with H3 the original layout still loses the deletion-INFO whenever VACUUM raises. (iv) Add a `--skip-headroom-check` flag — REJECTED, RULE-039 non-bypassable validation; operator runbook documents the manual `VACUUM INTO` reclaim procedure that mirrors the same constraint. (v) Migrate retention DELETE off SQLite entirely (e.g., daily-rollup architecture) — DEFERRED to a future sprint if `evaluation.db` ever grows beyond steady-state target by >50% (documented in runbook §"When to call this an architectural problem"). |
| **Scope** | Single session (Sprint 31.915 Session 1, 2026-04-28). Files modified: `argus/core/config.py` (new model + register overlay), `argus/strategies/telemetry_store.py` (config-driven + G1/G3/G4/G5), `argus/main.py` (wire config + register store with HealthMonitor), `argus/api/server.py` (wire config + register store), `argus/core/health.py` (new `register_evaluation_store` + `get_evaluation_db_health`), `argus/api/routes/health.py` (new `EvaluationDbHealth` model + extend `HealthResponse`), `config/evaluation_store.yaml` (new), `tests/test_telemetry_store.py` (8 new tests + 1-line surgical IMPROMPTU-10 monkeypatch adaptation), `tests/api/test_health.py` (2 new tests). 4 DEFs RESOLVED-IN-SPRINT (DEF-231/232/233/234), 1 OPEN-DEFERRED filed (DEF-235 for the test-zombie FD-pin pattern observed during the Apr 28 incident triage). |
| **Cross-References** | DEF-234 (resolved by this DEC); DEF-197 (IMPROMPTU-10 closure of unbounded-growth surface — DEC-389 supersedes the implicit policy without invalidating IMPROMPTU-10's mechanism; the 4-hour periodic-task pattern is preserved, only cadence + threshold defaults change); DEF-231 / DEF-232 / DEF-233 (companion observability + safety DEFs resolved in same session); DEF-235 (Apr 28 incident sibling — test-zombie FD pin); DEC-384 (standalone-overlay registry pattern that this entry follows); DEC-345 (separate-DB pattern for `evaluation.db`); DEC-328 (test discipline). Phase A diagnostic findings: `dev-logs/2026-04-28_retention-mechanism-diagnostic.md`. Operator runbook: `docs/operations/evaluation-db-runbook.md`. |
| **Impact** | Operator can now tune `evaluation.db` retention via `config/evaluation_store.yaml` without code edits. The retention loop is no longer silent — every iteration emits an INFO line (success or zero-deletion), and a vacuum failure cannot eat the deletion record because the deletion-INFO fires BEFORE the VACUUM attempt. Pre-VACUUM disk-headroom check structurally prevents the silent-ENOSPC class that triggered the Apr 27→28 silent-failure mode under the operator's tight disk environment. `/health.evaluation_db` gives operators independent confirmation of retention liveness with a 4-hour SLA (the periodic cadence). The existing IMPROMPTU-10 lifecycle pattern (4-hour periodic task + boot cleanup + close()-on-cancel) is preserved unchanged; only the cadence + threshold defaults are now config-driven. |
| **Status** | Active. |

---

*End of Decision Log v1.0*
*Next DEC: 390 (385 written above; 386 written above; 387 freed; 388 written above; 389 written above)*
*Last updated: 2026-04-29 (Sprint 31.915 — DEC-389 Config-Driven `evaluation.db` Retention; supersedes implicit DEF-197 closure without invalidating IMPROMPTU-10's mechanism)*
