...
> *Tier A operational context for Claude Code and Claude.ai. Last updated: April 1, 2026 (Sprint 32 doc sync — Parameterized Templates + Experiment Pipeline).*
...
**Sprints completed:** 1 through 29 + 21.6 + 25.9 + 27.5 + 27.6 + 27.65 + 27.7 + 27.75 + 27.8 + 27.9 + 27.95 + 28.5 + 28.75 + 29.5 + 32 (32 full sprints + 38 sub-sprints)
...
~~26~~ ✅ → ~~27~~ ✅ → ~~21.6~~ ✅ → ~~25.9~~ ✅ → ~~27.5~~ ✅ → ~~27.6~~ ✅ → ~~27.7~~ ✅ → ~~27.75~~ ✅ → ~~27.8~~ ✅ → ~~27.9~~ ✅ → ~~27.95~~ ✅ → ~~28~~ ✅ → ~~28.5~~ ✅ → ~~28.75~~ ✅ → ~~29~~ ✅ → ~~29.5~~ ✅ → ~~32~~ ✅ → **32.5 (Experiment Pipeline Completion + Visibility — DEF-131/132/134, 8 sessions)** → 31A (Pattern Expansion III — reach 15 strategies) → 30 (Short Selling + Parabolic Short — deferred until longs profitable) → 31.5 (Parallel Sweep Infrastructure) → 31B (Research Console — deferred per DEC-379) → 33 (Statistical Validation) → **33.5 (Adversarial Stress Testing)** → 34–35 (Systematic Search ★, Ensemble Analysis) → 36–42 (Cross-Family Search, Ensemble Orchestrator V2, Synapse, Learning Loop V2, Continuous Discovery, Performance Workbench). Sprint 32 (merged 32+32.5) complete: experiment registry, variant spawning, backtest pre-filter, promotion evaluator all delivered. Sprint 32.5 covers: DEF-131 (Experiments + Counterfactual UI), DEF-132 (exit params as variant dimensions), DEF-134 (BacktestEngine all 7 patterns), DEF-133 (Adaptive Capital Intelligence vision document). CPA consultation removed from all gates per DEC-380. Tax Intelligence Automation added to post-revenue horizon (DEC-380). Order Flow Model deferred to post-revenue (DEC-238). Historical data purchase deferred indefinitely (DEC-353); 96 months of OHLCV-1m available at $0 via XNAS.ITCH + XNYS.PILLAR (DEC-358). Full roadmap: `docs/roadmap.md`.
...
- **Experiment Pipeline (Sprint 32):** Complete variant lifecycle infrastructure for autonomous parameter optimization. **Pattern Factory** (`argus/strategies/patterns/factory.py`) — `build_pattern_from_config()` uses PatternParam introspection (no hardcoded switch statements), `get_pattern_class()` with lazy import + caching, `compute_parameter_fingerprint()` SHA-256 of canonical JSON of detection params (first 16 hex chars), `extract_detection_params()` discovers valid param names from `get_default_params()`. All 7 PatternModule patterns construct via factory in main.py. **ExperimentStore** (`argus/intelligence/experiments/store.py`) — SQLite in `data/experiments.db` (DEC-345 pattern), WAL mode, 3 tables (experiment_records, variant_definitions, promotion_events), 90-day retention. **VariantSpawner** (`argus/intelligence/experiments/spawner.py`) — reads `config/experiments.yaml`, deduplicates by fingerprint, registers variants with Orchestrator as shadow-mode PatternBasedStrategy instances. **ExperimentRunner** (`argus/intelligence/experiments/runner.py`) — grid generation from PatternParam metadata, BacktestEngine pre-filter per grid point; supports bull_flag + flat_top_breakout (2 of 7 patterns — DEF-134 tracks expanding to all 7). **PromotionEvaluator** (`argus/intelligence/experiments/promotion.py`) — autonomous shadow→live promotion and live→shadow demotion via Pareto comparison (5 metrics), configurable promotion thresholds, hysteresis guard, wired to SessionEndEvent. **ExperimentConfig** (`argus/intelligence/experiments/config.py`) — Pydantic model with `extra="forbid"`, config-gated via `experiments.enabled: false` (default). **REST:** 4 JWT-protected endpoints (`GET /api/v1/experiments`, `/{id}`, `/baseline/{pattern}`, `POST /run`). **CLI:** `scripts/run_experiment.py` standalone sweep execution (`--pattern`, `--cache-dir`, `--params`, `--dry-run`). **Parameter fingerprint end-to-end:** `_fingerprint_registry: dict[str, str | None]` on OrderManager, `register_strategy_fingerprint()` method, `config_fingerprint` column on trades table populated at trade close (non-PatternModule strategies get `None`). Config-gated: all features degrade gracefully when `experiments.enabled: false` (API returns 503, spawner/evaluator skip).
