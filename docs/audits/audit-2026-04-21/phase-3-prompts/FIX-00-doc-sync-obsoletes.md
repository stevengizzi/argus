...
- **Session:** `FIX-00` (full ID: `FIX-00-doc-sync-obsoletes`)
...
| Every resolved finding back-annotated in audit report with `**RESOLVED FIX-00-doc-sync-obsoletes**` | | |
...
Inputs:
- **Session spec:** the Findings to Fix section of this FIX-NN prompt (FIX-00-doc-sync-obsoletes)
...
3. **A one-line summary:** `Session FIX-00 complete. Close-out: {verdict}. Review: {verdict}. Commits: {SHAs}. Test delta: {baseline} -> {post} (net {±N}).`
