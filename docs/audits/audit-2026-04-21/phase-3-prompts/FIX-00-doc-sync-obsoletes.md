# Fix Session FIX-00-doc-sync-obsoletes: Mark OBSOLETE DEF entries in CLAUDE.md
...
**Files touched:** _(no source files — docs/ops only)_
**Safety tag:** `safe-during-trading`
**Theme:** Back-annotate CLAUDE.md so DEF entries that were superseded by later sprint work are clearly marked OBSOLETE. Pure documentation — no code, no tests, no configs.
...
marker (`audit(FIX-00): WIP — <reason>`) rather than leaving
uncommitted changes.
...
> In-memory ResultsCollector for parallel sweeps

**Impact:**

> N/A — superseded by Sprint 31.5 ProcessPoolExecutor approach

**Suggested fix:**

> Mark OBSOLETE in CLAUDE.md
...
4. Update the audit report row with `**RESOLVED FIX-00-doc-sync-obsoletes**`.
...
```
| ... | ~~description~~ **RESOLVED FIX-00-doc-sync-obsoletes** | ... |
```

For `read-only-no-fix-needed` findings that were verified, use
`**RESOLVED-VERIFIED FIX-00-doc-sync-obsoletes**` instead.
...
audit(FIX-00): doc-sync / obsolete DEF cleanup

Addresses audit findings:
- DEF-089 [COSMETIC]: In-memory ResultsCollector for parallel sweeps
...
- [ ] Audit report rows back-annotated with `**RESOLVED FIX-00-doc-sync-obsoletes**`
