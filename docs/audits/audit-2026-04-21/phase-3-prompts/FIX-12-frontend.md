...
- **Session:** `FIX-12` (full ID: `FIX-12-frontend`)
...
| Every resolved finding back-annotated in audit report with `**RESOLVED FIX-12-frontend**` | | |
...
Inputs:
- **Session spec:** the Findings to Fix section of this FIX-NN prompt (FIX-12-frontend)
...
3. **A one-line summary:** `Session FIX-12 complete. Close-out: {verdict}. Review: {verdict}. Commits: {SHAs}. Test delta: {baseline} -> {post} (net {±N}).`
