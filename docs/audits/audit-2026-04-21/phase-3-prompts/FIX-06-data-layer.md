...
| Every resolved finding back-annotated in audit report with `**RESOLVED FIX-06-data-layer**` | | |
...
Inputs:
- **Session spec:** the Findings to Fix section of this FIX-NN prompt (FIX-06-data-layer)
...
3. **A one-line summary:** `Session FIX-06 complete. Close-out: {verdict}. Review: {verdict}. Commits: {SHAs}. Test delta: {baseline} -> {post} (net {±N}).`
