...
| Every resolved finding back-annotated in audit report with `**RESOLVED FIX-08-intelligence-experiments-learning**` | | |
...
Inputs:
- **Session spec:** the Findings to Fix section of this FIX-NN prompt (FIX-08-intelligence-experiments-learning)
...
3. **A one-line summary:** `Session FIX-08 complete. Close-out: {verdict}. Review: {verdict}. Commits: {SHAs}. Test delta: {baseline} -> {post} (net {±N}).`
