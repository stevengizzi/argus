...
- **Session:** `FIX-17` (full ID: `FIX-17-claude-rules`)
...
| Every resolved finding back-annotated in audit report with `**RESOLVED FIX-17-claude-rules**` | | |
...
Inputs:
- **Session spec:** the Findings to Fix section of this FIX-NN prompt (FIX-17-claude-rules)
...
3. **A one-line summary:** `Session FIX-17 complete. Close-out: {verdict}. Review: {verdict}. Commits: {SHAs}. Test delta: {baseline} -> {post} (net {±N}).`
