...
- **Session:** `FIX-11` (full ID: `FIX-11-backend-api`)
...
| Every resolved finding back-annotated in audit report with `**RESOLVED FIX-11-backend-api**` | | |
...
Inputs:
- **Session spec:** the Findings to Fix section of this FIX-NN prompt (FIX-11-backend-api)
...
3. **A one-line summary:** `Session FIX-11 complete. Close-out: {verdict}. Review: {verdict}. Commits: {SHAs}. Test delta: {baseline} -> {post} (net {±N}).`
