...
| Every resolved finding back-annotated in audit report with `**RESOLVED FIX-14-docs-primary-context**` | | |
...
Inputs:
- **Session spec:** the Findings to Fix section of this FIX-NN prompt (FIX-14-docs-primary-context)
...
3. **A one-line summary:** `Session FIX-14 complete. Close-out: {verdict}. Review: {verdict}. Commits: {SHAs}. Test delta: {baseline} -> {post} (net {±N}).`
