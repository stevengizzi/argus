...
- **Session:** `FIX-18` (full ID: `FIX-18-deps-and-infra`)
...
| Every resolved finding back-annotated in audit report with `**RESOLVED FIX-18-deps-and-infra**` | | |
...
Inputs:
- **Session spec:** the Findings to Fix section of this FIX-NN prompt (FIX-18-deps-and-infra)
...
3. **A one-line summary:** `Session FIX-18 complete. Close-out: {verdict}. Review: {verdict}. Commits: {SHAs}. Test delta: {baseline} -> {post} (net {±N}).`
