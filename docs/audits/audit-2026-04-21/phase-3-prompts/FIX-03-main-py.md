...
- **Session:** `FIX-03` (full ID: `FIX-03-main-py`)
...
| Every resolved finding back-annotated in audit report with `**RESOLVED FIX-03-main-py**` | | |
...
Inputs:
- **Session spec:** the Findings to Fix section of this FIX-NN prompt (FIX-03-main-py)
...
3. **A one-line summary:** `Session FIX-03 complete. Close-out: {verdict}. Review: {verdict}. Commits: {SHAs}. Test delta: {baseline} -> {post} (net {±N}).`
