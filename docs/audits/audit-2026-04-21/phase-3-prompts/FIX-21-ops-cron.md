...
- **Session:** `FIX-21` (full ID: `FIX-21-ops-cron`)
...
| Every resolved finding back-annotated in audit report with `**RESOLVED FIX-21-ops-cron**` | | |
...
Inputs:
- **Session spec:** the Findings to Fix section of this FIX-NN prompt (FIX-21-ops-cron)
...
3. **A one-line summary:** `Session FIX-21 complete. Close-out: {verdict}. Review: {verdict}. Commits: {SHAs}. Test delta: {baseline} -> {post} (net {±N}).`
