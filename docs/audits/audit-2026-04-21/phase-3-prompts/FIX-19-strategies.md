...
- **Session:** `FIX-19` (full ID: `FIX-19-strategies`)
...
| Every resolved finding back-annotated in audit report with `**RESOLVED FIX-19-strategies**` | | |
...
Inputs:
- **Session spec:** the Findings to Fix section of this FIX-NN prompt (FIX-19-strategies)
...
3. **A one-line summary:** `Session FIX-19 complete. Close-out: {verdict}. Review: {verdict}. Commits: {SHAs}. Test delta: {baseline} -> {post} (net {±N}).`
