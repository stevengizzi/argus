...
| DEF-034 | Pydantic serialization warnings on review_verdict field | ~16 | PROMOTABLE | Cosmetic warning during test runs. Simple fix: `use_enum_values=True` on `SessionResult`, or emit `.value`. `safe-during-trading`, LOW severity, ~5-min fix. | none |
...
| DEF-089 | In-memory ResultsCollector for parallel sweeps | ~6 | OBSOLETE | **Superseded by Sprint 31.5 `ProcessPoolExecutor`**: fingerprint dedup + ExperimentStore writes are now pinned to main process (module-level worker function), removing the contention that motivated this DEF. The architecture described in DEF-089 ("per-run SQLite databases") was never built; the chosen approach removed the problem. Mark OBSOLETE. | Sprint 31.5 S1 |
...
| DEF-096 | Protocol type for duck-typed candle store reference | ~4 | SUPERSEDED-BY-FINDING → PROMOTABLE | P1-D1 L5 notes the same pattern recurs for CounterfactualTracker's `_store: object | None`. Batch fix: define `CandleStoreProtocol` + `CounterfactualStoreProtocol` in a shared typing module. `safe-during-trading`, LOW severity, ~30-min fix. | Sprint 27.65 S4 review, P1-D1 L5 |
| DEF-097 | Monthly cache update cron (populate_historical_cache.py --update) | ~3 | PROMOTABLE | Ops task. Pair with DEF-162 (its companion). Together: one cron that runs `populate_historical_cache.py --update` then `consolidate_parquet_cache.py --resume`. `safe-during-trading` (it's scheduling infrastructure, not runtime code). LOW severity, ~15-min fix (edit crontab + add doc entry). | P1-I audit noted the pair |
...
| DEF-106 | from_dict() in models.py contains ~8 assert statements | ~2 | SUPERSEDED-BY-FINDING → PROMOTABLE | P1-F1 finding #7 extends scope with `routes/counterfactual.py:204`. Batch fix: replace all `assert isinstance` with `if/raise TypeError`. `safe-during-trading` (test surface, production-code but unreachable in `-O` mode so the bug surfaces only when someone runs `python -O`). LOW severity, ~20-min fix. | P1-F1 #7 |
...
| DEF-162 | Monthly re-consolidation cron (pair with DEF-097) | ~0.5 | PROMOTABLE | Paired with DEF-097 as noted. Chain the crons. `safe-during-trading`. LOW severity. | DEF-097 pair |
