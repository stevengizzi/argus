...
| M-04 | [threshold_analyzer.py:115-151](argus/intelligence/learning/threshold_analyzer.py#L115-L151), [learning_service.py:509-532](argus/intelligence/learning/learning_service.py#L509-L532) | ~~`ThresholdAnalyzer._analyze_grade` can append **both** a "lower" and a "raise" `ThresholdRecommendation` for the same grade. `missed_opportunity_rate + correct_rejection_rate = 1.0` always, so when `missed > 0.50` both triggers fire. `_generate_proposals` then emits two `ConfigProposal` rows with the same `field_path` (`thresholds.<grade>`), one with value `current - 5.0` and one with `current + 5.0`. If an operator approves both, `apply_pending` applies them sequentially — second overwrites first and the drift guard sees twice the delta.~~ **RESOLVED FIX-08-intelligence-experiments-learning** via spec option (a). `_analyze_grade` now uses `if (correct < 0.50) "raise" elif (missed > 0.40) "lower" else no-rec`, emitting at most one recommendation per grade. Regression: `test_both_conditions_simultaneous_emit_only_raise` + `test_no_recommendation_when_neither_threshold_breached`. | Confusing operator UX (two contradictory proposals to review), and a subtle bug if both are approved. | Either (a) pick the stronger signal and emit exactly one recommendation per grade (e.g. `raise` when correct < 0.50, else `lower` when missed > 0.40); or (b) enforce at `apply_pending` that only one pending `thresholds.<grade>` proposal survives. Option (a) is cleaner. | safe-during-trading |
| M-05 | [config_proposal_manager.py:186-194, 424-452](argus/intelligence/learning/config_proposal_manager.py#L186-L194) | ~~`_redistribute_weights` proportionally adjusts non-changed weight dimensions to maintain sum-to-1.0, but the **redistribution deltas are never recorded** in `config_change_history`. Only the explicitly-changed dimension is persisted via `record_change`. `get_cumulative_drift(dim, window)` therefore undercounts drift on redistributed dimensions.~~ **RESOLVED FIX-08-intelligence-experiments-learning**. `apply_pending` now snapshots weights pre- and post-redistribution, and emits a separate `record_change(source="learning_loop_redistribution", proposal_id=...)` per redistributed dim. Regression: `test_redistribution_drift_visible_to_cumulative_drift_guard` exercises 3 sequential proposals and asserts cumulative drift on a never-explicitly-targeted dim equals the recorded redistribution sum. | Cumulative drift guard is evadable: repeatedly promote dim A up while dims B/C/D/E silently drift down via redistribution, and the guard never trips for B/C/D/E. Over months this could push the weight vector far from the initial anchor without operator awareness. | After applying each approved proposal, compute the actual delta for every redistributed dimension (compare pre- vs post-redistribution values) and call `record_change(..., source="learning_loop_redistribution", proposal_id=...)` for each. Drift guard then sees the full picture. | weekend-only |

## LOW Findings

| # | File:Line | Finding | Impact | Suggested Fix | Safety |
|---|-----------|---------|--------|---------------|--------|
| L-01 | [learning/models.py:409-423](argus/intelligence/learning/models.py#L409-L423) | ~~`_convert_datetimes` in `LearningReport.to_dict()` converts `isinstance(val, datetime)` only. A `date` (not `datetime`) field would slip through and cause `json.dumps` to fail at `learning_store.py:124` (no `default=str` there).~~ **RESOLVED FIX-08-intelligence-experiments-learning**. Guard widened to `isinstance(val, date)` (which also matches `datetime` since `datetime` is a `date` subclass); `datetime.isoformat()` still produces the richer string because Python dispatches to the subclass's overridden method first. Regression: `test_convert_datetimes_handles_date`. | Latent serialization crash if a future `LearningReport` / nested dataclass field uses `date`. No current failure — present dataclasses use `datetime` only. Same failure class as DEF-151. | Either widen the guard to `isinstance(val, (date, datetime))` (keep `datetime` first so `.isoformat()` picks the richer branch), or add `default=str` to the `json.dumps` call in `learning_store.save_report`. | safe-during-trading |
| L-02 | [promotion.py:365-413](argus/intelligence/experiments/promotion.py#L365-L413) | ~~`_build_result_from_shadow` and `_count_shadow_trading_days` issue **two independent** `counterfactual_store.query(..., limit=1000)` calls per variant per pass. If a strategy has >1000 shadow positions, the two queries return the same 1000 rows (DESC order), so day-count and r_multiple-count remain consistent. But the second query is redundant I/O.~~ **RESOLVED FIX-08-intelligence-experiments-learning**. Old `_build_result_from_shadow` + `_count_shadow_trading_days` collapsed into `_fetch_shadow_positions` (single query) + `_build_result_from_positions` (pure) + `_count_unique_days` (pure). `_evaluate_for_promotion` now fetches once and derives both the R-multiple list and the unique-day count. | Minor DB I/O waste during promotion evaluation (N variants × 2 queries instead of N). No correctness issue today. | Query once, compute both R-multiple list and unique day count from the returned rows. Avoids the second round-trip and future-proofs against the two limits ever diverging. | safe-during-trading |
| L-03 | [runner.py:468-520](argus/intelligence/experiments/runner.py#L468-L520) | ~~`workers > 1` branch catches `KeyboardInterrupt` and cleanly shuts down the executor with `cancel_futures=True`. A broad `Exception` raised inside `asyncio.as_completed(...)` (e.g. `aiosqlite.OperationalError` during `save_experiment`) would propagate past the try/else without `executor.shutdown(wait=False)`, leaving worker subprocesses running until the asyncio event loop exits.~~ **RESOLVED FIX-08-intelligence-experiments-learning**. Added `try/except KeyboardInterrupt/finally` around the `for coro in asyncio.as_completed(...)` loop with an `interrupted` flag driving `shutdown(wait=not interrupted, cancel_futures=interrupted)`. Exceptions of any type now reach the executor's shutdown; verified via `test_parallel_executor_shutdown_on_save_exception` (recording `ThreadPoolExecutor` asserts shutdown fires exactly once with `wait=True, cancel_futures=False` on save failure). | Potential orphan subprocess on rare write-path exceptions during parallel sweeps. Not triggered during normal operation (fire-and-forget store swallows writes). | Convert the `else: executor.shutdown(wait=True)` into a `finally:` that always runs. Or wrap the executor in a `with ProcessPoolExecutor(...) as executor:` context manager (which handles shutdown on any exit path). | weekend-only |
| L-04 | [learning_service.py:93-104](argus/intelligence/learning/learning_service.py#L93-L104) | ~~`register_auto_trigger` subscribes to `SessionEndEvent` with no teardown path — no `unsubscribe` call at shutdown. In practice both `LearningService` and `EventBus` live for the process lifetime, so this is not a leak today. Flag for visibility only; if `LearningService` ever becomes per-session (e.g. rebuilt on config reload), the subscription leaks across reloads.~~ **RESOLVED-VERIFIED FIX-08-intelligence-experiments-learning**. Re-grep confirmed `register_auto_trigger` remains a single-subscribe path; both `LearningService` and `EventBus` are process-lifetime singletons. Observation holds — no teardown needed today. Re-open if the lifecycle invariant changes. | No active bug. | If `LearningService` lifecycle ever changes, add an explicit unsubscribe in a `shutdown()` method. | read-only-no-fix-needed |
| L-05 | [experiments/store.py:765](argus/intelligence/experiments/store.py#L765) | ~~`_row_to_experiment` coerces `int(row["shadow_trades"])` directly. Column is `NOT NULL DEFAULT 0` in the DDL, so new rows are safe. Pre-DDL rows (none exist in production) would raise.~~ **RESOLVED-VERIFIED FIX-08-intelligence-experiments-learning**. Grep of DDL confirms `shadow_trades INTEGER NOT NULL DEFAULT 0` on both `experiments` (line 43) and `promotion_events` (line 72). No pre-DDL rows exist in production. Observation holds as documented risk only. | Theoretical only — no schema drift risk in the current DB. | No action needed. | read-only-no-fix-needed |
| L-06 | [experiments/runner.py:394-520](argus/intelligence/experiments/runner.py#L394-L520) vs [L525-L640](argus/intelligence/experiments/runner.py#L525-L640) | ~~Parallel and sequential sweep paths have significant code duplication — both handle unsupported-pattern FAILED records, both build `ExperimentRecord` with the same field set, both call `save_experiment` in the same shape. Two places to update if the record shape evolves.~~ **RESOLVED FIX-08-intelligence-experiments-learning**. Extracted two shared static helpers — `_make_unsupported_record(pattern_name, fingerprint, params, now)` and `_make_result_record(experiment_id, pattern_name, fingerprint, params, status, backtest_result, created_at, updated_at)` — used by both sweep paths. Parallel + sequential both call the same constructors; `save_experiment` is still called by each path at its own callsite (save is the path-specific step that differs in provenance). | Refactor risk, not a bug. | Extract a `_build_and_save_record(pattern_name, params, fingerprint, result, status, strategy_type, ...)` helper used by both paths. | safe-during-trading |
| L-07 | [learning_service.py:499](argus/intelligence/learning/learning_service.py#L499) | ~~`rationale` f-string includes `rec.correlation_trade_source or rec.correlation_counterfactual_source` — if both are `None` this prints `None`. Rationale is human-facing only; cosmetic.~~ **RESOLVED FIX-08-intelligence-experiments-learning**. Computed `correlation = (trade_src or cf_src or 0.0)` once before the f-string and formatted via `{correlation:+.4f}` so the rationale always renders a numeric value. | Rationale text on some proposals may show "correlation=None". | Format with a defensive `correlation_trade_source or correlation_counterfactual_source or 0.0`. | safe-during-trading |

## COSMETIC Findings

| # | File:Line | Finding | Impact | Suggested Fix | Safety |
|---|-----------|---------|--------|---------------|--------|
| C-01 | [runner.py:91-103](argus/intelligence/experiments/runner.py#L91-L103) | ~~`_run_single_backtest` does heavy local aliasing (`_asyncio`, `_date`, `_Path`, `_BEConfig`, `_StrategyType`, `_BacktestEngine`) with `# noqa: PLC0415`. The comment in the exception fallback explains why (pickle/subprocess loading), but the aliasing style is noisy and duplicates the module-level imports that already exist.~~ **RESOLVED FIX-08-intelligence-experiments-learning**. Removed all six local re-imports + their `noqa: PLC0415` markers; worker now uses module-scope `BacktestEngine`, `BacktestEngineConfig`, `StrategyType`, `asyncio`, `date`, `Path`. Rationale captured in the function docstring. (Side effect — required updating `test_run_single_backtest_passes_fingerprint` to patch `argus.intelligence.experiments.runner.BacktestEngine` rather than `argus.backtest.engine.BacktestEngine`, matching the pattern used by other parallel-path tests.) | Readability only. | Remove the aliasing, keep the explanatory comment, and let the noqa apply to the function import block rather than every statement. | safe-during-trading |
| C-02 | [config.py:70,80](argus/intelligence/experiments/config.py#L70-L80) | ~~`ExperimentConfig.variants` is typed `dict[str, list[dict[str, Any]]]` — accepts any shape. Each variant_def is later validated inside the spawner, but invalid top-level keys (e.g. typo'd `varients:` → entire dict ignored) pass Pydantic because `variants` is the catch-all. `extra="forbid"` helps on keys, not on value shape.~~ **RESOLVED FIX-08-intelligence-experiments-learning**. Added `VariantConfig` Pydantic model (`variant_id: str`, `mode: Literal["live","shadow"] = "shadow"`, `params: dict[str, Any]`, `exit_overrides: dict[str, float] \| None = None`, `extra="forbid"`); retyped `ExperimentConfig.variants` as `dict[str, list[VariantConfig]]`. Verified `config/experiments.yaml` round-trips through `ExperimentConfig(**yaml)` — all 22 variants parse cleanly. Production still consumes the YAML as a raw dict via `VariantSpawner` (no behavioural change there); the typed shape protects any future Pydantic-aware consumer (tests, startup wiring). | Low-visibility config typos. | Define a `VariantConfig` Pydantic model (`variant_id: str`, `mode: Literal["live","shadow"]`, `params: dict[str, Any]`, `exit_overrides: dict[str, float] \| None = None`) and retype `variants: dict[str, list[VariantConfig]]`. | safe-during-trading |
| C-03 | [learning/__init__.py:1-42](argus/intelligence/learning/__init__.py#L1-L42) | ~~`__init__.py` exports stores/service/models but **not** the three analyzers (`WeightAnalyzer`, `ThresholdAnalyzer`, `CorrelationAnalyzer`). Consumers (main.py, tests) import them by module path directly. Minor asymmetry.~~ **RESOLVED-VERIFIED FIX-08-intelligence-experiments-learning**. Grep of all consumers (8 files: main.py via api/server.py, scripts/run_learning_analysis.py, tests/intelligence/learning/*.py, tests/api/test_learning_api.py, learning_service.py) confirmed every analyzer reference uses the direct submodule path (`argus.intelligence.learning.{weight,threshold,correlation}_analyzer`). The asymmetry is consistent — analyzers are intentionally module-private and `__init__.py` exposes only the orchestration / data-model surface. No code change needed. | Readability. | Add analyzers to `__all__` and the import list, or document why they're not public API. | read-only-no-fix-needed |
| C-04 | [promotion.py:118-128](argus/intelligence/experiments/promotion.py#L118-L128) | ~~`_evaluate_for_promotion` hand-constructs a new `VariantDefinition` with `mode="live"` to append to the in-memory `live_variants` list after each promotion, duplicating every field. `VariantDefinition` is frozen, but `dataclasses.replace(shadow_variant, mode="live")` would be one line.~~ **RESOLVED FIX-08-intelligence-experiments-learning**. Replaced with `dataclasses.replace(shadow_variant, mode="live")`. Existing promotion tests (9) still pass — behavioural identity preserved since the dataclass is frozen and `replace` builds the same instance shape with `mode` overridden. | Readability. | Use `dataclasses.replace(shadow_variant, mode="live")`. | safe-during-trading |
| C-05 | [runner.py:244](argus/intelligence/experiments/runner.py#L244) | ~~`combos = list(itertools.product(*[param_ranges[k] for k in keys]))` materialises the entire cartesian product twice (once as combos, once in the list comprehension that follows). For large grids (near `_GRID_CAP = 500`) this is a minor memory blip.~~ **RESOLVED FIX-08-intelligence-experiments-learning**. Replaced with `[dict(zip(keys, combo)) for combo in itertools.product(*(param_ranges[k] for k in keys))]` — single materialisation, generator expression for the inner unpack. Same shape applied to the `exit_combos` path. | Negligible. | Iterate directly: `detection_grid = [dict(zip(keys, combo)) for combo in itertools.product(*(param_ranges[k] for k in keys))]`. | safe-during-trading |
| C-06 | [experiments/store.py:150-157](argus/intelligence/experiments/store.py#L150-L157) | ~~Idempotent `ALTER TABLE variants ADD COLUMN exit_overrides TEXT` swallows a bare `except Exception: pass`. Works in practice (SQLite raises on duplicate column), but hides other failure modes (permissions, disk full).~~ **RESOLVED FIX-08-intelligence-experiments-learning**. Narrowed to `except aiosqlite.OperationalError as exc` with a `"duplicate column name" in str(exc).lower()` sanity check; any other `OperationalError` (permissions, disk-full, locked-DB) re-raises so the failure surfaces. | Reviewability only. | Narrow to `except aiosqlite.OperationalError:` with a sanity check on the error message. | safe-during-trading |
