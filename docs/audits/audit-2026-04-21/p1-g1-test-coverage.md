...
| L4 | Data layer modules: `vix_data_service.py` (59%), `databento_data_service.py` (79%), `fmp_reference.py` (83%), `alpaca_data_service.py` (85%) | **RESOLVED FIX-06-data-layer** (partial — VIX refresh-path coverage added: `_fetch_range` happy path + both-empty raises, `fetch_incremental` skip-when-past-today, `initialize` yfinance-fallback; `fmp_reference` gains `known_symbols` + `_redact` tests; databento gains multi-month cache test. No FMP-fallback branch exists on VIXDataService — audit wording was misread; see close-out judgment call. Databento/fmp_reference/alpaca coverage improvements for scope-out files deferred.) — Data-service coverage is materially below the 90% target in `testing.md` §Core Logic. The `vix_data_service.py` gap in particular covers lines 556-685 — almost the entire `refresh()` / backfill code path. | VIX data is a RegimeClassifierV2 input; a silent regression in refresh would yield stale-but-plausible regime labels. Low severity because VIXDataService has explicit staleness self-disable guard per DEC-349. | Add tests for `VIXDataService.refresh()` happy path + FMP fallback branch. Data-service tests are well-established pattern — follow `test_fmp_reference.py`. | safe-during-trading |
...
- **C1** ~~Post-close circuit-breaker trigger path is uncovered~~ → **RESOLVED FIX-05-core-orchestrator-risk-regime**. `tests/core/test_risk_manager.py::TestPostCloseCircuitBreaker` adds 2 regression tests: (a) cumulative close losses past the daily limit fire the breaker via `PositionClosedEvent`, publish `CircuitBreakerEvent`, and reject subsequent signals; (b) early-exit when the breaker is already active (line 621) is a no-op with P&L still accumulating.
- **C2** ~~Two rejection paths in position-sizing are uncovered (risk_manager.py:386-394, 405-421)~~ → **RESOLVED FIX-05-core-orchestrator-risk-regime**. `TestPositionSizingRejectAfterReduction` adds 2 regression tests: (a) cash-reserve reduction below `min_position_risk_dollars` rejects with "cash reserve" + "$100 minimum" in the reason string; (b) buying-power reduction below floor rejects with "buying power" in the reason. Buying-power test uses a new `_LowBuyingPowerBroker` subclass to express margin-depleted state that SimulatedBroker V1 cannot model directly.
- **M5** ~~`test_def159` UTC/ET timezone mismatch~~ → **RESOLVED FIX-05-core-orchestrator-risk-regime**. `test_get_todays_pnl_excludes_unrecoverable` now captures `now` via `datetime.now(UTC).astimezone(ZoneInfo("America/New_York"))`.
- **M6** ~~`_make_vector()` hardcoded March 26 2026 default~~ → **RESOLVED FIX-05-core-orchestrator-risk-regime**. `tests/core/test_regime_vector_expansion.py::_make_vector()` default `computed_at` is now `datetime.now(UTC) - timedelta(hours=1)`.
- **M8** ~~`run_integrity_check()` zero-equity branch uncovered~~ → **RESOLVED FIX-05-core-orchestrator-risk-regime**. `TestIntegrityCheckZeroEquity::test_integrity_check_fails_on_zero_equity` covers the `account.equity <= 0` branch.

Remaining in-scope G1 findings (M1 pytest-xdist flake, M2 main.py coverage, M3 logging_config coverage, M4 30s flatten tests, M7 DEF-150 flake, M9 order-manager test consolidation, M10 coverage config, M11 entry-point coverage, L1-L5) are outside FIX-05's declared scope — they remain for subsequent Phase-3 sessions.
