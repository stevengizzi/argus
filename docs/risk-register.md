...
| **Severity** | Medium — downgraded from Critical at Sprint 31.91 D14 doc-sync (mechanism architecturally closed; observability gates met for criteria #1–#4; criterion #5 paper-session falsification pending). |
| **Likelihood** | Architecturally CLOSED via DEC-386 + DEC-385 + S3 retry side-check + S4 falsifiable validation infrastructure; pending empirical confirmation via 5 paper sessions clean post-seal (criterion #5). |
| **Description** | Bracket children placed via `parentId` only without explicit `ocaGroup`, combined with redundant standalone SELL orders from trail/escalation paths sharing no OCA group with bracket children, allowed multi-leg fill races. ARGUS's exit-side accounting was also side-blind in 3 surfaces (reconcile orphan-loop one-direction-only; reconcile call site stripped side info; DEF-158 retry path side-blind). On Apr 24 paper trading: 44 symbols / 14,249 shares of unintended short positions accumulated through gradual reconciliation-mismatch drift over a 6-hour session. |
| **Mitigation (in effect)** | **Sprint 31.91 SEALED 2026-04-28 — full architectural closure delivered.** (1) **DEC-386 (4-layer OCA-Group Threading + Broker-Only Safety)** — Sessions 0+1a+1b+1c, Tier 3 review #1 PROCEED 2026-04-27 — closes the bracket-internal fill race that produced ~98% of the blast radius per IMPROMPTU-11 (bracket children now in OCA group with `ocaType=1`; standalone-SELL paths thread the same group; broker-only paths cancel-then-SELL with propagation-await; `_handle_oca_already_filled` short-circuits redundant SELLs). (2) **DEC-385 (6-layer Side-Aware Reconciliation Contract)** — Sessions 2a+2b.1+2b.2+2c.1+2c.2+2d, sprint-close materialization 2026-04-28 — closes the residual detection-blindness mechanism via side-aware classification, `phantom_short_gated_symbols` SQLite table, and forensic-grade `phantom_short_override_audit` table. (3) **DEF-158 retry side-blindness fix (Session 3, anchor `a11c001`)** — 3-branch side-aware gate replaces unconditional SELL retry; new `phantom_short_retry_blocked` CRITICAL alert taxonomy. (4) **Falsifiable validation infrastructure (Session 4)** — `scripts/validate_session_oca_mass_balance.py` provides operator-runnable mass-balance assertion for end-of-session debrief; symbol-level audit + integration markers. (5) **DEC-388 (Alert Observability Architecture)** — Sessions 5a.1+5a.2+5b + Impromptus A+B+C + 5c+5d+5e — `phantom_short`/`phantom_short_retry_blocked`/`cancel_propagation_timeout` alerts now surface end-to-end in the Command Center via `AlertBanner` (cross-page) + `AlertToastStack` + Observatory `AlertsPanel` + audit endpoint. (6) IMPROMPTU-04's A1 fix (DEF-199) continues to refuse to amplify any residual short at EOD (1.00× signature, zero doubling) — defense-in-depth. (7) **Operator daily-flatten mitigation cessation criteria:** #1+#2+#3 SATISFIED post-S4; #4 (sprint sealed) MET 2026-04-28 at D14 doc-sync; #5 (5 paper sessions clean post-seal showing zero `unaccounted_leak` mass-balance rows + zero `phantom_short`/`cancel_propagation_timeout` alerts) PENDING — operator continues daily `scripts/ibkr_close_all_positions.py` flatten until criterion #5 satisfied. |
| **Owner** | Sprint 31.91 (`sprint-31.91-reconciliation-drift`; 25 implementation sessions + 2 Tier 3 reviews + 2 in-sprint hotfixes — SEALED 2026-04-28). Post-seal monitoring: operator paper-session debriefs through 5-session clean window. |
| **Status** | **MITIGATED — architecturally CLOSED via DEC-385 + DEC-386 + S3 + S4 + DEC-388.** Sprint 31.91 sealed at D14 doc-sync 2026-04-28. Live-trading transition still gated on: (a) criterion #5 (5 paper sessions clean post-seal); (b) DEF-208 live-trading test fixture (deferred to live-enable transition sprint); (c) DEF-211 D1+D2+D3 (Sprint 31.93 reconnect-recovery + STARTUP-ONLY runtime gate). See `docs/pre-live-transition-checklist.md` for the full gate list. |
| **Cross-references** | DEF-204 (CLAUDE.md DEF table); DEC-385 (decision-log.md, 2026-04-28 sprint-close); DEC-386 (decision-log.md, 2026-04-27); DEC-388 (decision-log.md, 2026-04-28 sprint-close — alert observability surface for the new alert taxonomy); IMPROMPTU-11 mechanism diagnostic (`docs/sprints/sprint-31.9/IMPROMPTU-11-mechanism-diagnostic.md`); Apr 24 debrief §A2/§C12 (`docs/sprints/sprint-31.9/debrief-2026-04-24-triage.md`); Sprint 31.91 sprint spec (`docs/sprints/sprint-31.91-reconciliation-drift/sprint-spec.md`); Tier 3 reviews #1 (2026-04-27) + #2 (2026-04-28 amended); `scripts/validate_session_oca_mass_balance.py` (S4 falsifiable validation harness); RSK-DEC-386-DOCSTRING (sibling — `reconstruct_from_broker` STARTUP-ONLY contract bounded by Sprint 31.93). |
...
| **Mitigation (in effect)** | (1) **ARGUS does not currently support mid-session reconnect at all.** The function has exactly one caller (`argus/main.py:1081`, gated by `_startup_flatten_disabled`). No existing reconnect path can trip the contract today. (2) DEF-211 D1 (CLAUDE.md DEF table) is filed as a **Sprint 31.93 sprint-gating item** — Sprint 31.93 (DEF-194/195/196 reconnect-recovery) is structurally forced to touch this function, and its spec MUST include the `ReconstructContext` introduction or it cannot be sealed. The DEF-211 scope was extended at Sprint 31.91 D14 doc-sync to D1+D2+D3 (D1 ReconstructContext parameter; D2 IMPROMPTU-04 startup-invariant gate refactor; D3 boot-time adoption-vs-flatten policy decision) — all three deliverables couple structurally and must land together in Sprint 31.93. (3) Architecture doc §3.7 OCA Architecture block + new §14 Alert Observability Subsystem (Sprint 31.91 D14) cross-reference the docstring at spec level so the contract is visible outside the source file. |
| **Owner** | Sprint 31.93 (DEF-194/195/196 reconnect-recovery + DEF-211 D1+D2+D3) — sprint-gating item. |
| **Status** | **OPEN — time-bounded by Sprint 31.93.** Sprint 31.91 sealed at D14 doc-sync 2026-04-28; Sprint 31.93 is now the next named horizon and inherits the runtime-gate conversion work. If Sprint 31.93 slips significantly, reassess: extract the STARTUP-only invariant into a runtime gate via a separate small impromptu before implementing any reconnect work. |
| **Cross-references** | DEC-385 (Sprint 31.91 Side-Aware Reconciliation Contract — L1 main.py call-site refactor is paired with this docstring contract); DEC-386 (Sprint 31.91 OCA architecture); DEC-388 (Sprint 31.91 Alert Observability Architecture — `cancel_propagation_timeout` alert is the surface that fires if the contract is violated); DEF-211 D1+D2+D3 (Sprint 31.93 sprint-gating; ReconstructContext parameter + IMPROMPTU-04 startup-invariant gate refactor + boot-time adoption-vs-flatten policy); DEF-194/195/196 (reconnect-recovery family); Tier 3 review #1 Focus Area 2 (2026-04-27); `argus/execution/order_manager.py::reconstruct_from_broker()` docstring; `docs/architecture.md` §3.7 OCA Architecture block + §14 Alert Observability Subsystem. |
...
### RSK — DEC-390 Amended Rule Coupling to Sprint 31.94 Reconnect-Recovery (RSK-DEC390-31.94-COUPLING)

| Field | Value |
|-------|-------|
| **ID** | RSK-DEC390-31.94-COUPLING |
| **Date Identified** | 2026-04-30 (Sprint 31.92 Tier 3 Review #2 verdict; **STATEMENT AMENDED at Tier 3 Review #3 verdict 2026-04-30**) |
| **Category** | Architectural Safety — Time-Bounded Contract |
| **Severity** | Medium |
| **Likelihood** | Low (no current trigger path; Sprint 31.94 design Phase B is the natural review surface) |
| **Description (AMENDED Tier 3 Review #3 2026-04-30)** | **Original statement (Tier 3 #2):** DEC-390's amended rule presumes H2 fails LOUD during Gateway disconnect; Sprint 31.94 reconnect-recovery may modify how `IBKRBroker.modify_order` surfaces during reconnect. **Amended statement (Tier 3 #3):** DEC-390's mechanism is Mechanism A (cancel-and-resubmit-fresh-stop), NOT H2 — Tier 3 #3 ELIMINATED-EMPIRICALLY H2 and H4 via DEF-242 (IBKR's broker policy categorically rejects `modify_order` against OCA group members under DEC-386 `ocaType=1` threading). Sprint 31.94's reconnect-recovery work must thread Mechanism A through any post-reconnect bracket-stop-price adjustment path. The "fail-loud during reconnect" framing is moot — Mechanism A doesn't use `modify_order`. The new coupling: Sprint 31.94's reconnect-recovery may introduce a post-reconnect re-amend pattern that MUST be routed through Mechanism A's cancel-and-resubmit-fresh-stop primitive, not a re-amend-via-modify primitive. DEF-244 is the canonical sprint-gating mechanism that codifies this binding. |
| **Mitigation (in effect)** | (1) DEC-385 (side-aware reconciliation) + DEC-386 (OCA-group threading) both retain their architectural integrity under Mechanism A; the H2-loud-fail dependency was structurally moot once H2 was eliminated. (2) DEF-244 (CLAUDE.md DEF table) is filed as Sprint 31.94 sprint-gating cross-sprint dependency — Sprint 31.94's Phase B DEC entry must explicitly cross-check that all post-reconnect stop-price adjustment paths are routed through Mechanism A. (3) Spike v2's `informational_axes_results.reconnect_window_amends` + `informational_axes_results.joint_reconnect_concurrent_amends` provide empirical grounding for the now-eliminated `modify_order` failure surface during reconnect; while moot for mechanism selection, the data remains useful to Sprint 31.94's design grounding (DEF-241 retained for historical traceability). (4) Sprint 31.91's OCA threading (DEC-386) is RETAINED unchanged per Tier 3 #3 Question 3 Answer — the `modify_order` incompatibility is a load-bearing architectural property, not a defect; rolling back to `ocaType=0` (S4b's escape hatch) is the operator's last-resort fallback if Mechanism A's gate fails. |
| **Time-bounded gating** | Revisit at Sprint 31.94 design Phase B. Sprint 31.94's Phase B DEC entry must explicitly cross-check that post-reconnect stop-price adjustment is routed through Mechanism A; if Sprint 31.94 introduces any path that does NOT use Mechanism A for that adjustment, file a follow-up DEF and revisit (likely Tier 3 review on the composability of Sprint 31.94's design with Mechanism A's primitive). |
| **Owner** | Sprint 31.94 (DEF-194/195/196 reconnect-recovery + DEF-211 D1+D2+D3) — sprint-gating cross-check item per DEF-244. |
| **Status** | **OPEN — time-bounded by Sprint 31.94 Phase B.** Sprint 31.92 sprint-close (D14 doc-sync) materializes DEC-390 with Mechanism A's binary-gate framing per Tier 3 #3 amendment; Sprint 31.94 sprint planning consumes both. |
| **Cross-references** | DEC-390 (Sprint 31.92 Pattern B sprint-close materialization — Mechanism A binary-gate rule); DEC-385 (Sprint 31.91 Side-Aware Reconciliation Contract); DEC-386 (Sprint 31.91 OCA-Group Threading — RETAINED unchanged at Tier 3 #3); DEF-194/195/196 (Sprint 31.94 reconnect-recovery family); DEF-241 (Tier 3 #2 cross-sprint dependency, retained for historical traceability — semantically superseded by DEF-244); DEF-244 (Tier 3 #3 cross-sprint dependency — Sprint 31.94 binding via Mechanism A); RSK-DEC386-MODIFY-INCOMPATIBILITY (sibling permanent constraint); `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-3-verdict.md` §Question 3 Answer / RSK Entries; `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-2-doc-sync-manifest.md`; `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-3-doc-sync-manifest.md`. |

---

### RSK — Mode D Contamination Recurrence (Spike Gate; RE-AFFIRMED at Tier 3 #3, applies to Unit 6 follow-on spike)

| Field | Value |
|-------|-------|
| **ID** | RSK-MODE-D-CONTAMINATION-RECURRENCE |
| **Date Identified** | 2026-04-30 (Sprint 31.92 Tier 3 Review #2 verdict; **RE-AFFIRMED at Tier 3 Review #3 verdict 2026-04-30** with status update for Unit 6 follow-on spike scope) |
| **Category** | Spike Gate Reliability |
| **Severity** | Medium-High |
| **Likelihood** | Low post-Cat-A.2 fix (3-branch side-aware `_flatten()` + pre-spike position-sweep refusal-to-start gate); residual risk from paper IBKR retaining residual positions across runs in undetectable ways (e.g., aggregated across `strategy_id` rows, pending-fill states broker has not yet confirmed) |
| **Description (statement preserved verbatim from Tier 3 #2)** | Even with Cat A.2 (DEF-237) + pre-spike position-sweep gate, paper IBKR may retain residual positions across runs in ways the spike harness cannot detect. **At Tier 3 Review #3 (2026-04-30), the statement applies to the Unit 6 follow-on spike's Mechanism A Mode-D-equivalent measurement** (was: spike v2's H1 Mode D measurement, semantically equivalent — both validate cancel-and-resubmit safety against residual broker-side positions). If Unit 6 N=100 Mode-D-equivalent produces `mechanism_a_zero_conflict_in_100 == false` driven by `position_state_inconsistency` conflicts, Mechanism A cannot be either qualified or disqualified, blocking S2a impl-prompt generation. The HARD GATE preserved verbatim from Tier 3 #2 (Decision 2) is structurally non-negotiable — any 1 conflict in 100 trials disqualifies Mechanism A — so a recurrence forces escalation A20 / Tier 3 Review #4. |
| **Mitigation (in effect)** | (1) DEF-237 fix (Cat A.2 three-branch side-aware `_flatten()`) is the primary structural defense; **Unit 6 inherits this verbatim** per the impl-prompt's Requirement 3. (2) Pre-spike position-sweep refusal-to-start gate at `main_async()` start refuses to launch if any nonzero position exists pre-run (operator must flatten manually with side-aware tooling — DEF-239 verified). **Unit 6's narrow scope (Mode-D-equivalent only, no axes (i)/(ii)/(iii)/(iv)) reduces the surface area where contamination can enter** — single measurement loop, no operator orchestration, no manual disconnect. (3) Unit 6 should LOG every Mode-D-equivalent `position_state_inconsistency` signature with full broker state diagnostic (full `get_positions()` snapshot before AND after each conflicting trial). This permits the operator to distinguish "spike-side leak" from "real Mechanism A cancel-and-resubmit race." (4) FAI #5's HARD GATE language preserved verbatim from Tier 3 #2 — any conflict count > 0 in 100 trials rules Mechanism A out regardless of other measurements. (5) DEF-243 fixes (errorEvent listener, FileHandler, isConnected guard) inherited as a hard prerequisite — the errorEvent listener also helps by capturing OCA-rejection signatures explicitly (rather than as silent measurement gaps). |
| **Time-bounded gating** | Revisit at Unit 6 close-out. If recurrence observed, A1 (escalation-criteria.md amended at this Tier 3 #3 mid-sync) re-fires AND A20 (NEW at Tier 3 #3) fires; operator arranges Tier 3 Review #4. |
| **Owner** | Sprint 31.92 Unit 6 follow-on spike close-out. |
| **Status** | **RE-AFFIRMED — gate-coupling, Unit 6 narrow scope reduces surface area.** Mitigation in effect via Cat A.2 fix inherited verbatim into Unit 6's impl-prompt; structural HARD GATE preserved verbatim. |
| **Cross-references** | DEF-237 (Cat A.2 fix; primary structural defense; inherited by Unit 6); DEF-239 (operator-tooling audit confirming side-aware daily-flatten available); DEF-243 (DEF-243 fixes are Unit 6 prerequisite); DEF-245 (Unit 6 spike scope DEF); FAI #5 (`docs/sprints/sprint-31.92-def-204-round-2/falsifiable-assumption-inventory.md` entry #5 — N=100 HARD GATE; relocated from S1a to Unit 6 at Tier 3 #3 mid-sync); A1 + A20 (`docs/sprints/sprint-31.92-def-204-round-2/escalation-criteria.md` — Tier 3 #3 escalation triggers); `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-2-verdict.md` §RSK-MODE-D-CONTAMINATION-RECURRENCE (original); `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-3-verdict.md` §RSK Entries (re-affirmation); `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-3-doc-sync-manifest.md`. |

---

### RSK — DEC-390 Verdict vs FAI #3 Compatibility Caveat (CLOSED-SUPERSEDED at Tier 3 #3)

| Field | Value |
|-------|-------|
| **ID** | RSK-VERDICT-VS-FAI-3-COMPATIBILITY |
| **Date Identified** | 2026-04-30 (Sprint 31.92 Tier 3 Review #2 verdict) |
| **Category** | Spec Encoding — Loose-Reading Robustness |
| **Severity** | Low-Medium |
| **Likelihood** | (Moot — see Status.) |
| **Description (preserved as historical narrative)** | DEC-390's amended rule (loose reading of FAI #3 — "rejection rate stable on production-reachable steady-state axis only") contradicts the strict reading of FAI #3 ("rejection rate stable under adversarial conditions" interpreted as "ALL four axes must remain ≤5% UB"). The Tier 3 Review #2 verdict resolved the ambiguity in favor of the loose reading — the strict reading was structurally degenerate (would have disqualified H2/H4/H1 simultaneously by axis (iii) at 100% UB by design). |
| **Status — CLOSED-SUPERSEDED at Tier 3 Review #3 (2026-04-30)** | The Tier 3 #2 ambiguity (loose vs strict reading of FAI #3) is moot because Tier 3 #3 ELIMINATED-EMPIRICALLY H2 and H4 (via DEF-242, OCA-categorical-rejection of `modify_order` against any OCA group member). Mechanism A's selection is now binary (gate-pass or gate-fail), not threshold-tiered across multiple axes. The underlying lesson — "primitive-semantics assumptions need empirical validation" — remains valid as a process invariant and is captured in the Tier 3 Review #1 self-falsifiability clause (FAI inventory). The axis-binding distinction in spec-by-contradiction.md §Out-of-Scope item 27 is preserved as historical narrative; spec-by-contradiction.md §Out-of-Scope item 28 (NEW at Tier 3 #3) records Mechanism B + Mechanism C elimination. |
| **Time-bounded gating** | None — closing at Tier 3 Review #3 verdict (2026-04-30). |
| **Owner** | (Closed.) |
| **Cross-references** | DEC-390 (Pattern B sprint-close materialization; Tier 3 #3 amends DEC-390's narrative to describe Mechanism A's binary gate); FAI #3 (`docs/sprints/sprint-31.92-def-204-round-2/falsifiable-assumption-inventory.md` entry #3 — amended at Tier 3 #3); spec-by-contradiction.md §"Out of Scope" item 27 (preserved as historical narrative; superseded in selection rule by item 28); spec-by-contradiction.md §"Out of Scope" item 28 (NEW at Tier 3 #3; Mechanism B + C eliminated); `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-2-verdict.md` §RSK-VERDICT-VS-FAI-3-COMPATIBILITY (original); `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-3-verdict.md` §RSK Entries (closure); `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-3-doc-sync-manifest.md`. |

---

### RSK — DEC-386 modify_order Incompatibility (Permanent Architectural Constraint)

| Field | Value |
|-------|-------|
| **ID** | RSK-DEC386-MODIFY-INCOMPATIBILITY |
| **Date Identified** | 2026-04-30 (Sprint 31.92 Tier 3 Review #3 verdict) |
| **Category** | Architectural Safety — Permanent Constraint (NOT time-bounded) |
| **Severity** | Medium |
| **Likelihood** | Confirmed (broker-side policy, observed empirically at 100% async-cancel rate across ~170 amends in spike v2 attempt 1, 2026-04-30) |
| **Description** | DEC-386's OCA-group threading (`ocaGroup` + `ocaType=1`) on bracket children is structurally incompatible with `modify_order` against any bracket child. IBKR's broker policy categorically rejects modification of OCA group members (Error 10326, "OCA group revision is not allowed"). Synchronous return is "accepted"; asynchronous cancellation arrives within milliseconds via the `errorEvent` callback. This is a permanent architectural constraint at the broker policy level, not a time-bounded contract. |
| **Implications** | All sprints touching post-placement bracket-order adjustments inherit this constraint. Specifically: Sprint 31.92 (current — Mechanism A absorbs it as PRIMARY DEFAULT); Sprint 31.93 (DEF-212 wiring is a load-bearing escape-hatch primitive, not cleanup, per Tier 3 #3 Question 3 Answer / S4b elevated priority); Sprint 31.94 (DEF-244 binds — reconnect-recovery uses Mechanism A); Sprint 35+ Learning Loop V2 (any dynamic-stop-price logic uses Mechanism A); any future sprint that introduces a new bracket-modification surface MUST explicitly verify Mechanism A compatibility at Phase A. |
| **Mitigation / escape hatch** | `IBKRConfig.bracket_oca_type = 0` (DEC-386's RESTART-REQUIRED rollback) reverts to no OCA bond on bracket children, recovering `modify_order` capability AT THE COST of losing OCA atomic cancellation (re-opening DEF-204's blast radius — per IMPROMPTU-11 diagnostic, ~98% of DEF-204's mechanism). This is operationally a last-resort escape hatch; DEF-212's wiring (Sprint 31.93) makes it a proper config primitive accessible to the operator with the dual-channel startup-warning + `--allow-rollback` CLI gate (S4b's responsibility, elevated to load-bearing at Tier 3 #3). |
| **Time-bounded gating** | **NONE — permanent.** This RSK does not have a "revisit at sprint X" trigger; it is a load-bearing project invariant. The asymmetric-risk-aware default (DEC-386's atomic-cancellation safety property dominates the modify-incompatibility cost) was selected at Sprint 31.91 and re-confirmed at Sprint 31.92 Tier 3 #3. |
| **Owner** | Cross-sprint architectural invariant. Each sprint that touches a bracket-modification surface owns the local Phase A compatibility verification. |
| **Status** | **PERMANENT — architectural invariant.** Materialized at Sprint 31.92 Tier 3 Review #3 mid-sync (2026-04-30). Propagates forward via DEF-244 (Sprint 31.94 binding); re-confirmed at every future sprint touching bracket modifications. |
| **Cross-references** | DEC-386 (Sprint 31.91 OCA-Group Threading + Broker-Only Safety — the architectural source); DEF-242 (architectural finding — empirical confirmation of the constraint); DEF-244 (Sprint 31.94 cross-sprint binding via Mechanism A); DEF-212 (Sprint 31.93 load-bearing rollback wiring); RSK-DEC390-31.94-COUPLING (sibling — coupling RSK with statement amended at Tier 3 #3); RSK-MECHANISM-A-UNPROTECTED-WINDOW (sibling — gate-coupling RSK for Mechanism A's correctness); `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-3-verdict.md` §Question 3 Answer; `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-3-doc-sync-manifest.md`. |

---

### RSK — Mechanism A Unprotected Window (Gate-Coupling to Unit 6)

| Field | Value |
|-------|-------|
| **ID** | RSK-MECHANISM-A-UNPROTECTED-WINDOW |
| **Date Identified** | 2026-04-30 (Sprint 31.92 Tier 3 Review #3 verdict) |
| **Category** | Mechanism Safety — Gate-Coupling to Unit 6 + S2a Defense-in-Depth |
| **Severity** | Medium-High (gate-failure path escalates to Tier 3 Review #4 via A20) |
| **Likelihood** | Low (target empirical signal: zero conflicts across N=100 trials; cancel-propagation latency p95 ≤ 2 s + fresh-stop-placement latency p95 ≤ 200 ms ⇒ ~2.2 s total bounded unprotected window per AMD-2-prime) |
| **Description** | Mechanism A's correctness depends on the unprotected window between (a) bracket-grouped stop cancel-propagation broker-side and (b) fresh outside-OCA stop placement confirmation. AMD-2-prime bounds this window by `cancel_propagation_timeout` ≤ 2 s + `fresh_stop_placement_p95` ≤ 200 ms (~2.2 s total). If the window proves operationally unsafe (any 1 conflict in 100 Mode-D-equivalent trials), Mechanism A is ineligible per the Tier 3 #3 binary HARD GATE; no viable mechanism remains in the H-class space (Mechanism B and Mechanism C eliminated per spec-by-contradiction.md §Out-of-Scope item 28); operator faces Tier 3 Review #4 disposition. Even if the gate passes, S2a's implementation MUST include defense-in-depth: HALT-ENTRY on `cancel_propagation_timeout` (per existing C-R2-1↔H-R2-2 coupling); operator-audit logging on every cancel-and-resubmit cycle; reconciliation cross-check during the unprotected window. |
| **Mitigation (in effect)** | (1) Unit 6 follow-on spike (DEF-245) exercises the binary HARD GATE (4 conditions) at N=100 BEFORE S2a impl — empirical falsification mechanism. (2) DEF-243 fixes (errorEvent listener, FileHandler, isConnected guard) inherited as a hard prerequisite — measurement infrastructure prevents silent measurement gaps that would obscure unprotected-window conflicts. (3) AMD-2-prime contract bounds the window structurally (2 s + 200 ms p95). (4) S2a impl-prompt's HALT-ENTRY-on-timeout regression test (per C-R2-1↔H-R2-2 coupling) is the in-process safety net; CL-3 cross-layer composition test under `SimulatedBrokerWithRefreshTimeout` fixture (S5c) is the additional cross-layer safety net. (5) Operator-audit logging at INFO (not WARN) on every cancel-and-resubmit cycle (AC1.6 amended at Tier 3 #3) enables post-hoc analysis of any production-paper anomaly. |
| **Time-bounded gating** | Revisit at Unit 6 close-out. If Mechanism A gate passes (`mechanism_a_zero_conflict_in_100 == true` AND latency thresholds), this RSK is downgraded to "Mechanism A operational risk" and tracked through paper-trading observation (Sprint 31.91 cessation criterion #5 + post-Sprint-31.92 paper sessions). If the gate fails (any 1 conflict in 100), this RSK is superseded by Tier 3 Review #4's verdict (operator architectural decision: rollback DEC-386 to `ocaType=0` via S4b's escape hatch, fundamental redesign, or DEF-204 Round 2 deferral). |
| **Owner** | Sprint 31.92 Unit 6 close-out → S2a impl-prompt finalization (gate-pass path) OR Tier 3 Review #4 (gate-fail path). |
| **Status** | **OPEN — gate-coupling to Unit 6 + S2a defense-in-depth.** Mitigation in effect via Unit 6's binary gate + DEF-243's measurement infrastructure + S2a's defense-in-depth design + S5c's CL-3 cross-layer composition test. |
| **Cross-references** | DEF-242 (architectural finding driving Mechanism A's selection); DEF-243 (sibling spike-harness measurement-gap fixes; Unit 6 prerequisite); DEF-245 (Unit 6 follow-on spike scope); RSK-DEC386-MODIFY-INCOMPATIBILITY (sibling permanent constraint — the architectural source of the `modify_order` incompatibility that drives Mechanism A's necessity); RSK-MODE-D-CONTAMINATION-RECURRENCE (sibling — gate-coupling RSK on the spike-side measurement reliability); A20 (`docs/sprints/sprint-31.92-def-204-round-2/escalation-criteria.md` — Mechanism A Unit 6 hard-gate failure trigger; NEW at Tier 3 #3); `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-3-verdict.md` §Question 2 Answer / Mechanism A in detail; `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-3-doc-sync-manifest.md`. |

---

*End of Risk & Assumptions Register v1.11*
*Last updated: 2026-04-30 (Sprint 31.92 Tier 3 Review #3 mid-sync — 2 new RSKs added: RSK-DEC386-MODIFY-INCOMPATIBILITY, RSK-MECHANISM-A-UNPROTECTED-WINDOW; 1 RSK statement amended: RSK-DEC390-31.94-COUPLING; 1 RSK re-affirmed with status update: RSK-MODE-D-CONTAMINATION-RECURRENCE; 1 RSK closed-superseded: RSK-VERDICT-VS-FAI-3-COMPATIBILITY; per `tier-3-review-3-doc-sync-manifest.md`)*
