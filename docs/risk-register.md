...
### RSK — DEC-389 Amended Rule Coupling to Sprint 31.94 Reconnect-Recovery

| Field | Value |
|-------|-------|
| **ID** | RSK-DEC389-31.94-COUPLING |
| **Date Identified** | 2026-04-30 (Sprint 31.92 Tier 3 Review #1 verdict) |
| **Category** | Architectural Safety — Time-Bounded Contract |
| **Severity** | Medium |
| **Likelihood** | Low (no current trigger path; Sprint 31.94 design Phase B is the natural review surface) |
| **Description** | DEC-389's amended rule (axis (i) production-reachable steady-state binds H2/H4/H1; axes (ii)/(iv) demoted to informational; axis (iii) deleted) presumes that H2 fails LOUD (returns rejected status, observable as `axis_i_wilson_ub` in steady-state vs `informational_axes_results` in reconnect-window) during Gateway disconnect. Sprint 31.94 reconnect-recovery work may modify how `IBKRBroker.modify_order` surfaces during a reconnect — for example, queueing pending modifications and replaying them post-reconnect with success status. If such a queue-and-replay design lands in Sprint 31.94, the H2 mechanism's protection against silent over-flatten during reconnect must be re-validated, because H2 may no longer fail-loud during reconnect under the new implementation. The Branch 4 + HALT-ENTRY coupling at C-R2-1↔H-R2-2 is precisely the structural defense that makes H2's reconnect-window failure operationally safe; that defense remains intact under the amendment but is sprint-31.94-design-dependent. |
| **Mitigation (in effect)** | (1) DEC-385 (side-aware reconciliation) + DEC-386 (OCA-group threading) both depend on H2 failing loud during reconnect to route via Branch 4 / HALT-ENTRY. (2) DEF-241 (CLAUDE.md DEF table) is filed as Sprint 31.94 sprint-gating cross-sprint dependency — Sprint 31.94's Phase B DEC entry must explicitly cross-check against DEC-389. (3) Spike v2's `informational_axes_results.reconnect_window_amends` + `informational_axes_results.joint_reconnect_concurrent_amends` provide empirical grounding for what H2's loud-fail surface looks like. (4) DEF-238 fail-loud instrumentation (Cat B.3 fix to spike harness) protects the signal's quality — operator skipping manual disconnect produces an `instrumentation_warning` JSON tag rather than silently corrupt data. |
| **Time-bounded gating** | Revisit at Sprint 31.94 design Phase B. If queue-and-replay is the chosen design, file a follow-up DEF here and revisit DEC-389 (likely a second Tier 3 review on the composability of DEC-389's loud-fail premise with Sprint 31.94's queue-and-replay). |
| **Owner** | Sprint 31.94 (DEF-194/195/196 reconnect-recovery + DEF-211 D1+D2+D3) — sprint-gating cross-check item per DEF-241. |
| **Status** | **OPEN — time-bounded by Sprint 31.94 Phase B.** Sprint 31.92 sprint-close (D14 doc-sync) materializes DEC-389 with this RSK as the time-bounded coupling artifact; Sprint 31.94 sprint planning consumes both. |
| **Cross-references** | DEC-389 (Sprint 31.92 Pattern B sprint-close materialization — rule amendment); DEC-385 (Sprint 31.91 Side-Aware Reconciliation Contract — depends on H2 loud-fail); DEC-386 (Sprint 31.91 OCA-Group Threading — depends on H2 loud-fail); DEF-194/195/196 (Sprint 31.94 reconnect-recovery family); DEF-241 (cross-sprint dependency lifting Sprint 31.94's design phase to consume informational axes); DEF-238 (spike-side fail-loud instrumentation); `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-1-verdict.md` §RSK-DEC389-31.94-COUPLING; `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-1-doc-sync-manifest.md`. |
...
| **Time-bounded gating** | Revisit at spike v2 close-out. If recurrence observed, A1 (escalation-criteria.md amended per this mid-sync) re-fires; operator arranges a second Tier 3 review. |
...
| **Cross-references** | DEF-237 (Cat A.2 fix; primary structural defense); DEF-239 (operator-tooling audit confirming side-aware daily-flatten available); FAI #5 (`docs/sprints/sprint-31.92-def-204-round-2/falsifiable-assumption-inventory.md` entry #5 — N=100 HARD GATE per Decision 2); A1 (`docs/sprints/sprint-31.92-def-204-round-2/escalation-criteria.md` — second-Tier-3 trigger if spike v2 INCONCLUSIVE); `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-1-verdict.md` §RSK-MODE-D-CONTAMINATION-RECURRENCE; `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-1-doc-sync-manifest.md`. |

---

### RSK — DEC-389 Verdict vs FAI #3 Compatibility Caveat
...
| **Description** | DEC-389's amended rule (loose reading of FAI #3 — "rejection rate stable on production-reachable steady-state axis only") contradicts the strict reading of FAI #3 ("rejection rate stable under adversarial conditions" interpreted as "ALL four axes must remain ≤5% UB"). The Tier 3 #1 verdict resolves the ambiguity in favor of the loose reading — the strict reading was structurally degenerate (would have disqualified H2/H4/H1 simultaneously by axis (iii) at 100% UB by design). Future reviewers (Round 3 adversarial review, M-R2-5 mid-sprint Tier 3, sprint-close adversarial review) should not re-litigate this ambiguity unless new evidence falsifies the loose reading's premise that axes (ii)/(iii)/(iv) test failure modes outside production reach or are addressed by separate sprints. |
| **Mitigation (in effect)** | (1) DEC-389 (Pattern B sprint-close materialization) explicitly documents the loose-vs-strict resolution rationale verbatim. (2) sprint-spec.md §Hypothesis Prescription amended halt-or-proceed gate language encodes the binding (`axis_i_wilson_ub`) cleanly. (3) spec-by-contradiction.md §"Out of Scope" item 27 records the strict reading as superseded. (4) falsifiable-assumption-inventory.md FAI #3 amended — text is now self-consistent with the loose reading. (5) escalation-criteria.md A1 amended — INCONCLUSIVE under DEC-389 amended rule is the new threshold. (6) The verdict's full rationale lives at `tier-3-review-1-verdict.md` § "Why the loose reading is correct, not the strict reading." |
| **Time-bounded gating** | Revisit if spike v2's axis (i) result is itself ambiguous (e.g., 4.5%–5.5% UB straddling the H2/H4 threshold). In that case, the loose reading's robustness becomes a gating question — operator may need a second Tier 3 review to confirm the H2/H4 threshold itself rather than re-litigating the axis-binding rule. |
| **Owner** | Sprint 31.92 spike v2 close-out (no action required if axis (i) UB falls cleanly into one of the H2/H4/H1 buckets); Sprint 31.92 sprint-close DEC-389 materialization. |
| **Status** | **OPEN — spec-encoding caveat.** Resolved at the spec layer; awaits empirical confirmation via spike v2 axis (i) result. |
| **Cross-references** | DEC-389 (Pattern B sprint-close materialization); FAI #3 (`docs/sprints/sprint-31.92-def-204-round-2/falsifiable-assumption-inventory.md` entry #3 — amended this mid-sync); spec-by-contradiction.md §"Out of Scope" item 27 (this mid-sync); sprint-spec.md §Hypothesis Prescription amended halt-or-proceed gate language (this mid-sync); escalation-criteria.md A1 amended (this mid-sync); `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-1-verdict.md` §RSK-VERDICT-VS-FAI-3-COMPATIBILITY; `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-1-doc-sync-manifest.md`. |

---

*End of Risk & Assumptions Register v1.10*
*Last updated: 2026-04-30 (Sprint 31.92 Tier 3 Review #1 mid-sync — three new RSKs added: RSK-DEC389-31.94-COUPLING, RSK-MODE-D-CONTAMINATION-RECURRENCE, RSK-VERDICT-VS-FAI-3-COMPATIBILITY; per `tier-3-review-1-doc-sync-manifest.md`)*
