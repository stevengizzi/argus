...
- **Concentration limit approve-with-modification (DEC-249):** Single-stock concentration limit (5% of equity) uses approve-with-modification instead of hard rejection. Shares reduced to fit within limit. If reduced position falls below 0.25R floor, rejected entirely. Resolves systemic issue where risk-based sizing produced positions exceeding concentration limit for mid-to-high priced stocks with tight stops.
