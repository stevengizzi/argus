...
| Field | Value |
|---|---|
| **Refreshed at** | 2026-04-28, end-of-Session-3 verdict |
| **Anchor commit** | `0f7415f` (S3 Tier 2 review CLEAR + impl `a11c001`); CI run `25035589472` GREEN |
| **Sessions complete** | 0, 1a, 1b, 1c, 2a, 2b.1, 2b.2, 2c.1, 2c.2 (+ impromptu DEF-216 hotfix), 2d, 3 |
| **Tier 3 reviews complete** | #1 (PROCEED) |
| **Active session** | None — between sessions; cleared to proceed to Session 4 |
| **Sprint phase** | Track A complete (OCA architecture, DEC-386 at Tier 3 #1); Track B complete (side-aware reconciliation, DEC-385 at S2d); **Track C complete (DEF-158 retry side-check, single-session at S3)**; Session 4 (mass-balance + IMSR replay + decomposed live-enable gate) pending; Track D (alert observability, 6 sessions + Tier 3 #2 + DEC-388) pending |
...
- **Mode:** HITL on `main`
- **Primary defects:** DEF-204 (reconciliation drift / phantom-short mechanism — **all architectural layers LIVE post-S3; falsifiable validation pending S4**), DEF-014 (alert observability gap)
- **Operational mitigation:** Operator runs `scripts/ibkr_close_all_positions.py` daily — REQUIRED, NOT OPTIONAL per Apr 27 evidence. **Conservative cessation criteria #1 + #2 SATISFIED post-S3**; criteria #3-#5 still pending.
...
**Sprint cumulative delta:** +94 pytest, 0 Vitest.

**Test_main.py baseline drift:** Pre-existing 31 pass / 5 skip / 8 fail (vs assumed 39 pass + 5 skip baseline). Noted at S2d; not a Sprint 31.91 regression. Documented in CLAUDE.md DEF-048 lineage.
...
### Filed pre-Sprint 31.91, status changes from S3

| DEF | Status |
|---|---|
| DEF-204 | Reconciliation drift / phantom-short mechanism — **ALL architectural layers LIVE post-S3**: detection (S2b.1) + count-filter (S2b.2) + entry-gate (S2c.1) + auto-clear (S2c.2) + manual override (S2d) + retry-path side-check (S3). Falsifiable validation pending at S4 (mass-balance assertion + IMSR replay) AND first OCA-effective paper session (Apr 28+) |
| DEF-014 | Alert observability gap — RESOLVES at Session 5e |
| **DEF-158** | Flatten retry side-blindness — **RESOLVED in S3 (commit `a11c001`)**. 3-branch side-aware gate at `_check_flatten_pending_timeouts:3276-3341` mirrors IMPROMPTU-04 EOD A1 idiom. Anti-regression test `test_def158_retry_qty_mismatch_long_uses_broker_qty` explicit. Mark RESOLVED in CLAUDE.md at sprint-end doc-sync |
...
| DEF-199 | EOD Pass 2 A1 fix preserved — UNCHANGED across S0–S3 |

### Anticipated but NOT filed

| Anticipated DEF | Reason not filed |
|---|---|
| DEF-208 | No fresh evidence requiring filing through S3 |
...
| 2b.2 cross-component coupling on `_broker_orphan_last_alerted_cycle` | S2b.2 (production wiring deferred S5a.1+) | `HealthMonitor._order_manager` reads dict via `getattr` defensive pattern |
...
| **DEF-158 closure** | S3 | 3-branch side-aware gate at `_check_flatten_pending_timeouts:3276-3341`; explicit anti-regression test for qty-mismatch path |
| **`phantom_short_retry_blocked` alert taxonomy** (sibling to `phantom_short`) | S3 | New CRITICAL alert type at `:3293-3314` for DEF-158 retry path; distinct from S2b.1/2b.2/2d's `phantom_short` so S5a.2 policy table can route separately |
| **OCA-EXEMPT comment refresh for `_check_flatten_pending_timeouts`** | S3 | Pre-Session-3 "Session 3 will branch on side" → post-Session-3 "Session 3 has now landed the upstream side gate". `# OCA-EXEMPT:` regression-guard marker preserved (4/4 tests pass in `test_oca_threading_completeness.py`) |
...
| Pass 1 retry SELL detection at `order_manager.py:1777` is sibling to Pass 2 but NOT alerted with `phantom_short`. S3 added a sibling alert type (`phantom_short_retry_blocked`) at a DIFFERENT site (DEF-158 retry); the original Pass 1 retry consistency-gap is contextually unchanged | LOW (consistency gap) | S2b.2 Edge Case 3 | Future session can extend taxonomy |
...
| Spec do-not-modify line range `:1670-1750` for `order_manager.py` does NOT actually contain SELL-detection branching post-S0-S2d accumulation. **RE-VALIDATED at S3:** spec D6 anchored insertion at `:2384` but function had drifted to `:3171+` — S3 adapted per RULE-038 (function ID stable; line offset drifted) | LOW (spec-anchor discrepancy) | S2b.2 RULE-038 #3 + S3 B5-informational | Future impl prompts should reference structural anchor |
...
| `OrderManager.stop()` does not await `_pending_gate_persist_tasks` | LOW (same-family follow-on) | S2c.1 review concern #2 | NOT addressed in S2d or S3; eligible for future graceful-shutdown improvement |
...
| Test 6 anchors on S2c.1's rehydration log not S2d's lifespan log | LOW (small docstring/anchor mismatch) | S2d reviewer soft observation #1 | Behaviorally correct; future S2d-specific test could lock the lifespan-layer log explicitly |
...
| Audit table `phantom_short_override_audit` has no retention policy by design (append-only forever per Sprint 31.91 retention spec) | OBSERVATIONAL (intentional) | S2d reviewer soft observation #4 | Forensic-grade audit log; rare events; small row size; correct trade-off |
| **NEW (S3):** Defensive `try/except` around `event_bus.publish` at Branch 2 marked `# pragma: no cover - defensive` | OBSERVATIONAL (intentional) | S3 reviewer Notable Item #2 | Idiomatic and safe; protects against subscriber misbehavior |
| **NEW (S3):** Test 3 (Branch 3) asserts no `SystemAlertEvent` at all (stronger than spec) | OBSERVATIONAL (intentional defensive) | S3 reviewer Notable Item #3 | Future regression adding an alert to Branch 3 will require deliberate test update |
...
|---|---|---|
| **Daily-flatten cessation** | Conservative criteria #1 + **#2 SATISFIED post-S3**; criteria #3-#5 pending | Sprint-end + 5 paper-session-clean window |
...
| Emitter-site line-number drift in `order_manager.py` | TRACKED — RE-VALIDATED at S3 (function ID stable; line offset drifts) | S5a.1 pre-flight grep canonical |
| 9 emitter sites at S3 close (all populate `metadata` from day one — zero S5a.1 migration burden for the 7 newer sites) | TRACKED for S5a.1 grep | S5a.1 |
...
| `OrderManager.stop()` graceful-shutdown await of `_pending_gate_persist_tasks` | Watch | NOT addressed in S2d/S3; eligible for future improvement |
...
| Two pre-existing operator stashes (`stash@{0}` from `8ccac67` 2026-04-22; `stash@{1}` from `2a59083` 2026-03-26) | OPERATOR ACTION | At operator's convenience |
| **NEW (S3):** Original Pass 1 retry SELL consistency gap at `:1777` (sibling to Pass 2 but not `phantom_short`-alerted) — S3 added `phantom_short_retry_blocked` at a DIFFERENT site (DEF-158 retry), so the original carry-forward is contextually unchanged | OBSERVATIONAL | Future session |
...
| HIGH #1 auto-resolution policy | Explicit per-alert-type predicates (8 entries — now 5 phantom-short-family alert types post-S3 plus 3 others) | Session 5a.2 |
| HIGH #4 decomposed live-enable gate | 4 criteria (1, 2, 3a, 3b) | Session 4 (NEXT) |
...
- `phantom_short_retry_blocked` (from DEF-158 retry — 1 source, severity `critical`) **NEW S3**
...
S3 added no new SQLite surfaces. Sprint-end doc-sync MUST surface both tables in `architecture.md` storage table + `live-operations.md` operational reference.
...
14. ⏳ **Session 4 (mass-balance categorized + IMSR replay + decomposed live-enable gate)** ← NEXT (falsifiable validation gate)
...
- Architecture.md §3.3 + §3.7 OCA architecture (already done; verify) + §14 alert observability (new) + §X side-aware reconciliation contract per DEC-385 (new) + retry-path side-check section (new, S3)
...
- `live-operations.md` updates: OCA architecture operations section (already done; verify), DEF-214 EOD verification fix runbook, DEF-215 deferred revisit trigger, **conservative daily-flatten cessation criteria + revisit decision**, alert observability acknowledgment runbook, broker-orphan-LONG M2 exp-backoff schedule documentation, **5-alert-type phantom_short-family taxonomy** (reconciliation / health.integrity_check / eod_flatten / startup.aggregate / startup.per-symbol / **DEF-158 retry-blocked**), **B22 runbook for operator override** (already done in S2d; verify), **DEF-158 retry-blocked operator runbook** (new for S3 — `phantom_short_retry_blocked` alert response procedure)
...
- Pass 1 retry SELL `phantom_short` alert extension follow-up (consistency-gap at `:1777`, distinct from S3's DEF-158 retry block — deferred to future session)
