...
| **S5c — DEF-220 (`acknowledgment_required_severities` consumer wiring)** | S5c | Option A — REMOVAL applied; field deleted from `AlertsConfig` + both YAML overlays; per-alert-type `PolicyEntry.operator_ack_required` is canonical home; Tier 3 #2 pre-recommended this disposition |
...
| **NEW (S5c):** No explicit WS-reconnect logic in `useAlerts` hook | NOTE (F1, non-blocking) | S5c reviewer F1 | Matches `useArenaWebSocket` precedent + spec sketch; REST polling (5s) covers operationally; revisit in 5e or post-31.91 if needed |
| **NEW (S5c):** 5xx ack-error path not behaviorally tested | NOTE (F4, non-blocking) | S5c reviewer F4 + closeout | Intentional — error UI lands in S5d's modal |
| **NEW (S5c):** `getToken() === null` early-return branch not tested | NOTE (non-blocking) | S5c closeout 312-316 | Operationally fires only when JWT cleared mid-session; REST query's 401 path handles redirect-to-login |
