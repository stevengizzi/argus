...
## Unit 6 (`unit-6-mechanism-a-followon-spike`): Mechanism A Follow-on Spike — INSERTED per Tier 3 Review #3 verdict 2026-04-30

**Scope (1 sentence):** Phase A diagnostic follow-on spike against fresh
OCA-grouped brackets on paper IBKR (account U24619949, single client) —
N=100 Mode-D-equivalent cancel-and-resubmit-fresh-stop trials measuring
unprotected-window event count + cancel-propagation latency + fresh-stop
placement latency, gating Mechanism A's selection per the binary halt
gate (4 conditions: `mechanism_a_zero_conflict_in_100 == true` AND
`cancel_propagation_p50_ms ≤ 1000` AND `cancel_propagation_p95_ms ≤ 2000`
AND `fresh_stop_placement_p95_ms ≤ 200`).

**Creates:**
- `scripts/spike_def204_mechanism_a_followon.py` (~200 LOC; smaller than
  S1a v2 by ~60–70% — no axes (i)/(ii)/(iii)/(iv); single Mode-D-equivalent
  measurement mode)
- `scripts/spike-results/spike-def204-mechanism-a-followon-results.json`
  (autogenerated; required keys: `status` ∈ {`PROCEED`, `INCONCLUSIVE`},
  `selected_mechanism` ∈ {`mechanism_a`, `null`},
  **`mechanism_a_zero_conflict_in_100: bool`** (binary HARD GATE),
  `cancel_propagation_p50_ms`, `cancel_propagation_p95_ms`,
  `fresh_stop_placement_p95_ms`, `unprotected_window_observations: list`,
  `trial_count`, `spike_run_date`)

**Modifies:** none (read-only spike against paper IBKR + ARGUS broker
code).

**Integrates:** N/A — outputs gate Phase D impl prompt for S2a + S2b.
**Hypothesis-prescription gate logic** (binary, per DEC-390 amended at
Tier 3 #3): if all four binary-gate conditions hold AND
`status == "PROCEED"`, S2a proceeds with Mechanism A. Otherwise A20
fires; Tier 3 Review #4 disposition required.

**Parallelizable:** **FALSE** (no other spike or implementation runs
concurrent with Unit 6's operator-execution). DEF-243 fixes (errorEvent
listener, `logging.FileHandler`, `isConnected()` precondition gate) MUST
land first as a separate Unit before Unit 6's spike script is authored.

**Pre-flight reads (5):**
1. `scripts/spike_def204_round2_path1.py` (S1a v2 reference pattern; the
   prior spike harness; Unit 6 inherits its pre-spike position-sweep
   refusal-to-start gate but drops axes (i)/(ii)/(iii)/(iv))
2. `argus/execution/ibkr_broker.py::cancel_all_orders` +
   `place_bracket_order` + `place_order` surfaces (anchor by class +
   method names; structural)
3. `argus/execution/order_manager.py::_trail_flatten` (the production
   call site whose mechanism Unit 6 is validating)
4. `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-3-verdict.md`
   §Question 2 Answer / Mechanism A in detail / Follow-on spike scope
5. `sprint-spec.md` §"Hypothesis Prescription" (Mechanism A binary gate
   language)

**New tests:** 0 (spike script produces JSON artifacts; Tier 2 review
of script BEFORE operator execution per Tier 3 #2 workflow protocol gap
recommendation #1; close-out validates artifact schema against the
...
| Factor | Count | Points |
|--------|------:|------:|
| New files | 1 (script; JSON autogenerated) | +2 |
| Modified files | 0 | 0 |
| Pre-flight reads | 5 | +5 |
| New tests | 0 | 0 |
| Complex integration (3+ components) | NO (read-only spike) | 0 |
| External API (live IBKR paper) | YES | +3 |
| Large new file (>150 lines) | YES (~200 LOC) | +2 |
| **TOTAL** | | **12 (Medium)** |

**Note on scope narrowing:** Unit 6 is approximately 60–70% smaller in
scope than S1a v2 because: (a) no axes (i)/(ii)/(iii)/(iv) measurement
modes (those validated H2's `modify_order` rejection rate, which is
moot under Mechanism A); (b) single Mode-D-equivalent loop; (c) no
operator orchestration (no manual disconnect, no joint conditions —
just N=100 fresh-bracket trials). Total LOC delta vs. baseline spike
harness is a net REDUCTION in measurement modes balanced against the
DEF-243 measurement infrastructure additions (errorEvent listener,
FileHandler, isConnected guard).

**AC coverage:** Generates evidence for AC1.1 through AC1.6 Mechanism A
mechanism choice + AC1.5 AMD-2-prime framing + AC1.6 operator-audit
logging cadence. Output JSON consumed by S2a/S2b impl prompts.

**Halt-or-proceed gate language for S2a/S2b impl prompt** (verbatim from
Sprint Spec §"Hypothesis Prescription", Mechanism A binary gate per
Tier 3 Review #3 2026-04-30):

> Mechanism A is selected if and only if all four conditions hold across
> the Unit 6 follow-on spike (N=100 trials):
...
>
> **HARD GATE:** any 1 conflict in 100 → Mechanism A NOT eligible;
> escalation A20 fires; Tier 3 Review #4 disposition required.

**Dependencies:** Unit B (DEF-243 fixes — errorEvent listener,
FileHandler, isConnected guard) MUST commit before Unit 6's spike script
is authored. DEF-243's three fixes are a hard prerequisite. Sprint 31.91
SEALED at HEAD; paper IBKR Gateway operational; market closed
(non-safe-during-trading — run pre-market or after-hours, single
`clientId=1`).

**Cross-references:** DEF-242 (architectural finding — H2/H4 elimination
that drove the need for this spike); DEF-243 (sibling spike-harness
measurement-gap fixes that land first); DEF-245 (Unit 6 spike scope
DEF); RSK-MECHANISM-A-UNPROTECTED-WINDOW (gate-coupling RSK; revisit at
Unit 6 close-out); `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-3-verdict.md`
§Question 2 Answer.
...
**Scope (1 sentence):** Implement Mechanism A (cancel-and-resubmit-fresh-stop;
PRIMARY DEFAULT per Tier 3 Review #3 verdict 2026-04-30 — H2 and H4
ELIMINATED-EMPIRICALLY by DEF-242) in `_trail_flatten`; 6 tests covering
the canonical BITU race, OCA-already-filled preservation, AMD-8 + AMD-4
guard preservation, AMD-2-prime framing, and operator-audit logging per
AC1.6 (every cycle, INFO).
