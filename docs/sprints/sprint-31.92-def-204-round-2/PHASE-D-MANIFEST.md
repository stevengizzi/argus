...
- **`config/order_management.yaml` → `config/order_manager.yaml`** — disposition/spec drafts cited `config/order_management.yaml`; actual repo-HEAD path is `config/order_manager.yaml`. Path-drift correction applied across all Phase D artifacts (S3a impl/review, session-breakdown, phase-a-reentry-findings, design-summary, adversarial-review-input-package-round-3, falsifiable-assumption-inventory). S3a's grep-verify guard language ("or wherever the operator-facing overlay lives — verify during pre-flight") is preserved as defensive scaffolding.
