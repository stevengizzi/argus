...
|---|---------|-------------------|
| A1 | **Spike returns `status: INCONCLUSIVE` under DEC-390 binary Mechanism A gate** in `scripts/spike-results/spike-def204-round2-path1-results.json` OR (if Unit 6 follow-on spike has been authored) `scripts/spike-results/spike_def204_mechanism_a_followon.json`. After Tier 3 Review #3 (2026-04-30) the rule structure is no longer threshold-tiered across H2/H4/H1 — H2 and H4 are ELIMINATED-EMPIRICALLY by IBKR's broker-side categorical rejection of `modify_order` against OCA group members (DEF-242), and only Mechanism A (cancel-and-resubmit-fresh-stop, formerly H1) remains viable. The amended rule is a binary gate: Mechanism A is selected if and only if all four conditions hold across the Unit 6 follow-on spike (N=100): `mechanism_a_zero_conflict_in_100 == true`; `cancel_propagation_p50_ms ≤ 1000`; `cancel_propagation_p95_ms ≤ 2000`; `fresh_stop_placement_p95_ms ≤ 200`. A1 fires when any of those four conditions is not met OR the artifact returns `status: INCONCLUSIVE` for any other reason. **Per Tier 3 Review #2 verdict 2026-04-30 (initial threshold-tier framing) → superseded by Tier 3 Review #3 verdict 2026-04-30 (binary Mechanism A gate framing).** Spike v2 attempt 1 produced INCONCLUSIVE for the right reason (DEC-390 rule output) but framed it through DEF-243's measurement-gap lens; the underlying empirical signal was the OCA-categorical-rejection finding now codified as DEF-242. | Halt before S2a impl. Operator arranges Tier 3 Review #4 (per the new A20 trigger). The S2a/S2b implementation prompts MUST NOT be generated until Tier 3 #4 confirms a viable mechanism OR confirms the spike harness needs further amendment. Per protocol §"Hypothesis Prescription": "Do NOT ship a Phase B fix that doesn't address the Phase A finding." Cross-references: RSK-MECHANISM-A-UNPROTECTED-WINDOW (`docs/risk-register.md`); RSK-MODE-D-CONTAMINATION-RECURRENCE (`docs/risk-register.md` — re-affirmed); `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-3-verdict.md`; `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-3-doc-sync-manifest.md`. |
...
| A20 | **(NEW per Tier 3 Review #3 verdict 2026-04-30 — Mechanism A Unit 6 hard-gate failure)** Unit 6 follow-on spike returns `mechanism_a_zero_conflict_in_100 == false` (any 1 conflict in 100 trials) OR any of the three latency thresholds is breached (`cancel_propagation_p50_ms > 1000`, `cancel_propagation_p95_ms > 2000`, `fresh_stop_placement_p95_ms > 200`). Mechanism A is the only remaining viable mechanism in the H-class space after H2/H4 were ELIMINATED-EMPIRICALLY by Tier 3 #3 (DEF-242, OCA-categorical-rejection). | **Halt before S2a impl. Escalate to Tier 3 Review #4.** No viable mechanism remains in the H-class space. Operator faces a hard architectural decision: (a) roll back DEC-386 to `ocaType=0` (loses OCA atomic cancellation; re-opens DEF-204's blast radius — operationally a last-resort escape hatch via S4b's `IBKRConfig.bracket_oca_type` rollback); (b) fundamentally rethink the dynamic-stop-price pattern (Mechanism B = cancel-bracket / submit-fresh-bracket and Mechanism C = OCA-membership manipulation are explicitly out of scope per spec-by-contradiction.md §Out-of-Scope item 28 — Tier 3 #4 would have to relitigate); (c) defer DEF-204 Round 2 closure to a later sprint pending broker-side IBKR API evolution. Cross-references: RSK-MECHANISM-A-UNPROTECTED-WINDOW (`docs/risk-register.md`); DEF-242 (architectural finding driving the elimination of H2/H4); DEF-245 (Unit 6 follow-on spike scope); `docs/sprints/sprint-31.92-def-204-round-2/tier-3-review-3-verdict.md` §Question 2 Answer / HARD GATE language. |
...
6. **DEC-391 cannot be coherently materialized at sprint-close** — e.g., the four layers (Path #1 + Path #2 + ceiling + DEF-212 rider) end up addressing different mechanisms than spec-anticipated, OR adversarial review reveals an architectural conflict between L1 and L3 that requires splitting DEC-391 into multiple DECs. **Default disposition: split the DEC and proceed with sprint-close** — this is not a sprint-abort trigger but a doc-sync complication. Listed here for completeness.
...
- DEC-391 written below in `docs/decision-log.md` with all 4 layers and complete cross-references AND structural-closure framing (NOT aggregate percentage claims) per AC6.3.
...
- `docs/dec-index.md` updated with DEC-391 entry.
...
- 7 new RSKs filed: RSK-DEC-391-AMEND (if H2) OR RSK-DEC-391-CANCEL-AWAIT-LATENCY (if H1/H4 with H1 fallback path active), RSK-DEC-391-FINGERPRINT, RSK-CEILING-FALSE-POSITIVE, RSK-RECONSTRUCTED-POSITION-DEGRADATION (severity MEDIUM-HIGH; time-bounded by Sprint 31.94 D3), RSK-SUPPRESSION-LEAK, RSK-FAI-COMPLETENESS (Tier 3-class), RSK-CROSS-LAYER-INCOMPLETENESS (Tier 3-class).
