...
| 3 | **(AMENDED Tier 3 Review #3 2026-04-30.)** IBKR's broker policy under DEC-386's `ocaType=1` threading on bracket children **categorically rejects** `modify_order` against any OCA group member (Error 10326, "OCA group revision is not allowed"; sync-accept then async-cancel within 1–6 ms via `errorEvent` callback; observed at 100% async-cancel rate across ~170 amends in spike v2 attempt 1, 2026-04-30 — see DEF-242). H2 (modify_order PRIMARY DEFAULT) and H4 (hybrid amend) are therefore ELIMINATED-EMPIRICALLY. Mechanism A (cancel-and-resubmit-fresh-stop, formerly H1, now PRIMARY DEFAULT per Tier 3 #3) replaces the threshold-tiered H2/H4/H1 selection. The binding empirical question is no longer `modifyOrder` rejection rate but Mechanism A's Mode-D-equivalent gate: zero conflicts in N=100 cancel-and-resubmit cycles AND cancel-propagation latency thresholds (p50 ≤ 1000 ms, p95 ≤ 2000 ms) AND fresh-stop placement latency (p95 ≤ 200 ms). The Tier 3 #2 axis (i)/(ii)/(iii)/(iv) framing is structurally MOOT for Mechanism A selection; axes (ii) and (iv) are retained in the prior spike v2 JSON artifact only as informational input to Sprint 31.94 reconnect-recovery design per DEF-241 / RSK-DEC390-31.94-COUPLING. Cross-reference: spec-by-contradiction.md §Out-of-Scope item 28 (Mechanism B + Mechanism C eliminated); DEF-242 (architectural finding); DEF-245 (Unit 6 follow-on spike scope). | **Unit 6 follow-on spike (Mechanism A Mode-D-equivalent, N=100).** Narrow spike script `scripts/spike_def204_mechanism_a_followon.py` (or equivalent) creates fresh OCA-grouped brackets, runs N=100 cancel-and-resubmit-fresh-stop cycles, measures cancel-propagation latency + fresh-stop-placement latency + unprotected-window-event count. **Binary halt-or-proceed gate (per DEC-390 amended at Tier 3 #3):** all four conditions must hold — `mechanism_a_zero_conflict_in_100 == true`, `cancel_propagation_p50_ms ≤ 1000`, `cancel_propagation_p95_ms ≤ 2000`, `fresh_stop_placement_p95_ms ≤ 200`. **HARD GATE:** any 1 conflict in 100 → escalate to Tier 3 Review #4 (escalation-criteria.md A20). DEF-243 fixes (errorEvent listener, FileHandler, isConnected guard) MUST land first. | **PARTIALLY-VERIFIED — H2 falsified by Tier 3 #3 (2026-04-30); Mechanism A verification pending Unit 6 follow-on spike.** Will become **fully verified** on green Unit 6 run with all four binary-gate conditions met. |
...
| 5 | **(AMENDED Tier 3 Review #3 2026-04-30 — RELOCATED from S1a to Unit 6 follow-on spike; semantic content preserved.)** `cancel_all_orders(symbol, await_propagation=True)` synchronously confirms all bracket-child cancellations broker-side before returning. **Mechanism A** (cancel-and-resubmit-fresh-stop) — the now-PRIMARY-DEFAULT mechanism after H2/H4 ELIMINATED-EMPIRICALLY per Tier 3 #3 — depends on this. Without broker-side cancel-propagation confirmation, the fresh-stop-placement step can collide with still-active bracket children (OCA conflict, locate suppression, or both). The unprotected window between cancel-propagation and fresh-stop-placement is bounded by AMD-2-prime (`cancel_propagation_timeout` ≤ 2 s + fresh-stop placement ≤ 200 ms p95 = ~2.2 s total). | **Unit 6 follow-on spike (Mode-D-equivalent for Mechanism A, N=100; replaces the prior S1a strengthened cancel-then-immediate-SELL stress framing).** Script `scripts/spike_def204_mechanism_a_followon.py` (or equivalent): (a) cancel bracket-grouped stop via `cancel_all_orders(symbol, await_propagation=True)` against fresh OCA-grouped brackets created by the spike itself; (b) immediately place fresh outside-OCA stop (≤10ms gap); (c) measure across **N=100 trials** that no `position_state_inconsistency`, OCA conflict, locate suppression, or unintended-fill events surface during the unprotected window. **HARD GATE preserved verbatim from prior FAI #5: if even 1 trial in 100 exhibits a conflict, Mechanism A is NOT eligible regardless of cancel-propagation latency measurements** — escalation A20 fires; Tier 3 Review #4 disposition required. JSON output schema includes `mechanism_a_zero_conflict_in_100: bool` (replaces prior `h1_propagation_zero_conflict_in_100`) + `cancel_propagation_p{50,95}_ms` + `fresh_stop_placement_p95_ms` + `unprotected_window_observations[]`. DEF-243 fixes (errorEvent listener, FileHandler, isConnected guard) MUST land first. | **unverified — falsifying spike scheduled in Unit 6 follow-on spike (was S1a strengthened sub-spike pre-Tier-3-#3).** Will become **falsified** on green Unit 6 run if `mechanism_a_zero_conflict_in_100 == true`. Hard-gated; any non-zero conflict count rules Mechanism A out and there is no remaining viable mechanism in the H-class space. |
