...
The target: **sustainable 5–10%+ monthly returns on deployed capital** at the $100K–$500K scale, with the system running autonomously during US market hours.
...
Sprint 31.9's closure unblocked four post-31.9 horizons. **`post-31.9-reconciliation-drift` was renamed Sprint 31.91 and SEALED 2026-04-28** (see entry below). The remaining three horizons are queued; build-track queue position is operator-decided.

#### Sprint 31.91 — Reconciliation Drift / Phantom-Short Fix + Alert Observability ✅ COMPLETE (April 28, 2026)
**Actual:** ~6 days (25 implementation sessions + 2 Tier 3 reviews + 2 in-sprint hotfixes). +189 pytest, +47 Vitest. Anchor commit `4c737d5`.

**Delivered:**
- **DEC-386 (4-layer OCA-Group Threading + Broker-Only Safety)** — Sessions 0+1a+1b+1c, Tier 3 #1 PROCEED 2026-04-27. Closes ~98% of DEF-204's cascade mechanism. (1) `Broker.cancel_all_orders(symbol, *, await_propagation)` ABC extension + `CancelPropagationTimeout` exception. (2) Bracket OCA grouping with `ocaType=1` on children only + `ManagedPosition.oca_group_id` + `_is_oca_already_filled_error` helper. (3) Standalone-SELL OCA threading on 4 paths + `redundant_exit_observed` SAFE marker + grep regression guard with `# OCA-EXEMPT:` exemption mechanism. (4) Broker-only safety via cancel-then-SELL/wire on 3 paths + `reconstruct_from_broker` STARTUP-ONLY contract docstring + `_emit_cancel_propagation_timeout_alert` shared helper.
- **DEC-385 (6-layer Side-Aware Reconciliation Contract)** — Sessions 2a+2b.1+2b.2+2c.1+2c.2+2d, sprint-close materialization 2026-04-28. (L1) `argus/main.py` call-site builds typed dict from `broker.get_positions()` including side. (L2) `SystemAlertEvent.metadata: dict[str, Any] | None` schema field (closes DEF-213 jointly with S5a.1). (L3) `OrderManager` Pass 1 retry path side-aware 3-branch SELL detection. (L4) Phantom-short gate emits side-aware classifications + persists to `phantom_short_gated_symbols` SQLite table; new `phantom_short_retry_blocked` CRITICAL alert. (L5) EOD verify polling treats SHORT-side as phantom-short candidates. (L6) `phantom_short_override_audit` SQLite table records every operator override for forensic-grade replay.
- **Session 3 — DEF-158 retry side-blindness fix** — 3-branch side-aware gate replaces unconditional SELL retry; new `phantom_short_retry_blocked` CRITICAL alert taxonomy (anchor `a11c001`).
- **Session 4 — Falsifiable validation infrastructure** — `scripts/validate_session_oca_mass_balance.py` provides operator-runnable mass-balance assertion for end-of-session debrief; symbol-level audit + integration markers.
- **DEC-388 (Alert Observability Architecture)** — Sessions 5a.1+5a.2+5b + Impromptus A+B+C + 5c+5d+5e, sprint-close materialization 2026-04-28. Resolves DEF-014 (PRIMARY DEFECT, October 2025). 5-layer architecture: producer (15 emitter sites populating `SystemAlertEvent.metadata`), consumer (HealthMonitor + 13-entry `POLICY_TABLE` + AST regression guard), storage (`data/operations.db` 5 tables + restart recovery + migration framework), REST + WebSocket (4 REST endpoints + JWT-authenticated `/ws/v1/alerts`), frontend (`useAlerts` hook + `AlertBanner` + `AlertToastStack` + `AlertAcknowledgmentModal` + `AlertsPanel` Observatory + `AppShell.tsx` cross-page mount).
- **Migration framework universal adoption (Impromptu C)** — All 8 ARGUS SQLite DBs are now framework-managed (operations + catalyst + evaluation + regime_history + learning + vix_landscape + counterfactual + experiments). Sprint-spec D16 fulfilled.
- **In-sprint hotfixes** — `c36a30c` (DEF-216 ET-midnight rollover flake) and `4c737d5` (post-S5e API catalog freshness regeneration for `GET /api/v1/alerts/{alert_id}/audit`).

**State after:** DEF-014 FULLY RESOLVED via end-to-end alert observability pipeline. DEF-204 mechanism architecturally CLOSED via DEC-386 + DEC-385 + S3 + S4. **Daily-flatten mitigation cessation criteria:** #1+#2+#3 SATISFIED post-S4; #4 (sprint sealed) MET 2026-04-28; #5 (5 paper sessions clean post-seal) PENDING — operator continues `scripts/ibkr_close_all_positions.py` until criterion #5. **Sprint 31.92 (DEF-204 Round 2) inherits DEF-212 as rider (`_OCA_TYPE_BRACKET` constant drift), with the Path #1/#2 fixes as primary scope** — see Apr 28 paper-session debrief. Sprint 31.94 inherits DEF-211 D1+D2+D3 (`ReconstructContext` parameter + IMPROMPTU-04 startup-invariant gate refactor + boot-time adoption-vs-flatten policy decision) + RSK-DEC-386-DOCSTRING bound + DEF-194/195/196 reconnect-recovery. DEF-222 (subscribe-before-rehydrate audit) deferred to first producer-wiring sprint.

**Tests:** 5,080 → 5,269 pytest (+189), 866 → 913 Vitest (+47).
**DECs:** DEC-385 (Side-Aware Reconciliation Contract), DEC-386 (OCA-Group Threading + Broker-Only Safety), DEC-388 (Alert Observability Architecture). DEC-387 freed.
**DEFs:** 13 RESOLVED-IN-SPRINT (DEF-014, DEF-158 follow-on, DEF-213, DEF-214, DEF-216, DEF-217, DEF-218, DEF-219, DEF-220, DEF-221, DEF-223, DEF-224, DEF-225). 5 new opportunistic LOW deferrals filed (DEF-226/227/228/229/230). 1 deferred from Tier 3 #2 spawn (DEF-222, bounded by first producer-wiring sprint).

**Tier 3 reviews:** #1 PROCEED 2026-04-27 (DEC-386 OCA architecture). #2 PROCEED-with-conditions AMENDED 2026-04-28 (DEC-388 deferred to sprint-close per Pattern B; 9 new DEFs filed, 8 RESOLVED-IN-SPRINT via Impromptus A+B+C and S5c).

**Carry-forward to next campaign's RETRO-FOLD:** F.1 — DEC-328 full-suite-at-Tier-1 process gap surfaced via post-S5e catalog freshness hotfix; documented in `docs/process-evolution.md` for next sprint planning.

**Sprint folder canonical artifacts:** `docs/sprints/sprint-31.91-reconciliation-drift/{def-disposition-matrix.md, work-journal-register.md, pre-impromptu-doc-sync-manifest.md, tier-3-review-1-verdict.md, tier-3-review-2-verdict.md}` plus 25 session closeouts + 25 session reviews.

#### Sprint 31.92 — DEF-204 Round 2 (Trail-Stop / Bracket-Stop Race + Locate-Rejection Hold + DEF-212 Rider)
**Target:** 1 session, sibling cleanup

Wire `IBKRConfig.bracket_oca_type` into `OrderManager.__init__` and replace the `_OCA_TYPE_BRACKET = 1` module constant in `argus/execution/order_manager.py:82` per Tier 3 #1 Concern B. Sibling cleanup: relocate `_is_oca_already_filled_error` from `ibkr_broker.py` to `broker.py` and rename to `is_oca_already_filled_error` (Tier 3 #1 Concern A). Most natural rider on the `post-31.9-component-ownership` OrderManager-construction-refactor session (the two are cross-coupled — DEF-212 fix requires `OrderManager.__init__` to accept `IBKRConfig`, which is the broader component-ownership refactor's prerequisite).

#### Sprint 31.94 — Reconnect Recovery + RejectionStage + DEF-211 D1+D2+D3
**Target:** 3+ sessions, sprint-gating items

Reconnect-recovery work: DEF-194 (IBKR `ib_async` stale position cache after reconnect), DEF-195 (`max_concurrent_positions` count diverges from broker state after disconnect-recovery), DEF-196 (DEC-372 stop-retry-exhaustion cascade events). **Sprint-gating items inherited from Sprint 31.91 Tier 3 #1 + Apr 27 paper-session debrief Findings 3+4 (DEF-211 EXTENDED SCOPE):** D1 — convert `reconstruct_from_broker()` STARTUP-ONLY docstring contract to runtime `ReconstructContext` parameter (closes RSK-DEC-386-DOCSTRING). D2 — IMPROMPTU-04 startup invariant gate refactor (distinguish 3 boot states: clean fresh boot, STARTUP_FRESH with broker-only positions, RECONNECT_MID_SESSION). D3 — boot-time adoption-vs-flatten policy decision for broker-only LONG positions + `max_concurrent_positions` broker-only-longs accounting fix. **Most likely first firing surface for DEF-222 audit** (subscribe-before-rehydrate predicate-handler ordering, which becomes load-bearing once `IBKRReconnectedEvent` producer lands). Plus RejectionStage enum split: DEF-177 + DEF-184 + DEF-014 IBKR emitter TODOs. Adversarial review required.

#### Sprint 31.93 — Component Ownership Consolidation (was `post-31.9-component-ownership`)
**Target:** 2–3 sessions

Architectural refactor migrating component construction from `argus/api/server.py` `_init_*` lifespan phases into `argus/main.py` Phase 9.x so ArgusSystem becomes the sole owner of `CatalystStorage`, `SetupQualityEngine`, `DynamicPositionSizer`, `ExperimentStore`, and `LearningStore`. `argus/api/server.py` becomes a pure REST/WS adapter that reads pre-populated components from `AppState`. Resolves DEF-175 + DEF-182 + DEF-193 + DEF-201 + DEF-202 and structurally resolves DEF-172 (which was closed as RESOLVED-VERIFIED via behavioral proof; this sprint removes the duplication at the source). Relocates the DEF-173 `LearningStore.enforce_retention()` wiring next to the existing `ExperimentStore` retention call in `main.py`. **Absorbed from Sprint 31.91 (per `def-disposition-matrix.md` Section C):** Impromptu B Concern 2 (`_log_post_start_symbology_size` task lifecycle hygiene — DEF-202 sibling-class), Impromptu C LOW #1 (CounterfactualStore `variant_id` ALTER bare-except FIX-08 alignment), S2c.1 reviewer concern #2 (`OrderManager.stop()` does not await `_pending_gate_persist_tasks` — graceful-shutdown lifecycle hygiene, DEF-202 sibling-class). Component-ownership is the natural sprint home for all three.

Pre-sprint discovery captured in `docs/sprints/post-31.9-component-ownership/DISCOVERY.md`. **Entry criteria satisfied** by Sprint 31.9 closure (2026-04-24); Sprint 31.91 closure (2026-04-28) further reinforced via 6 main.py scoped exceptions across S2a/S2c.1/S2d/S5a.1/S5a.2 (× 2). Follow sprint-planning protocol (Phase A deep-think → Phase B design summary → Phase C spec → Phase D execution) — do NOT execute as impromptus.

#### post-31.9-reconnect-recovery-and-rejectionstage
**Target:** 3 sessions

Reconnect-recovery work: DEF-194 (IBKR `ib_async` stale position cache after reconnect), DEF-195 (`max_concurrent_positions` count diverges from broker state after disconnect-recovery missed fill callbacks), DEF-196 (DEC-372 stop-retry-exhaustion cascade events). Plus RejectionStage enum split: DEF-177 (add `MARGIN_CIRCUIT`) and DEF-184 (split `RejectionStage` into rejections + `TrackingReason`). Plus DEF-014 IBKR emitter TODOs. Plus Apr 21 audit F-04. **Likely first firing surface for DEF-222** (subscribe-before-rehydrate predicate-handler audit — becomes load-bearing once `IBKRReconnectedEvent` producer lands here). Adversarial review required for the reconnect-recovery sessions; standard Tier 2 for the RejectionStage enum work. Discovery: `docs/sprints/post-31.9-reconnect-recovery-and-rejectionstage/DISCOVERY.md`. **Note:** The Sprint 31.93 entry above documents the DEF-211 D1+D2+D3 sprint-gating items that this sprint must include alongside the reconnect-recovery work — Sprint 31.94 IS this sprint, with the extended-scope inheritance from Sprint 31.91 Tier 3 #1 + Apr 27 paper-session debrief Findings 3+4.

#### Sprint 31.95 — Alpaca Retirement (was `post-31.9-alpaca-retirement`)
...
**Note:** Deferred from original Sprint 31 position. Reframed in Sprint 31.75 as a **Variant Factory** — the primary interface for generating configs, running quick-reject backtests (seconds per config), and pushing survivors to shadow. The Research Console visualization features (heatmaps, 3D parameter landscape) are secondary to the operational workflow. **Prerequisite cleared (Sprint 31.85):** DEF-161 resolved via `scripts/consolidate_parquet_cache.py` — derived `data/databento_cache_consolidated/` (~24K per-symbol files) unblocks DuckDB-backed SQL queries once the operator repoints `config/historical_query.yaml`. `resolve_symbols_fast.py` (pyarrow sampling) remains available for symbol-resolution use cases.
...
| **Network latency** | Low | All | Minimal latency from US East Coast. No longer a structural concern for any strategy duration. |
